...
Recent history uses Conventional Commit style with scopes, for example `feat(observability): surface watcher state age` and `fix(observability): pin francis observer to one session`. Current maintainer workflow is serialized direct-on-`main`: sync `origin/main`, assign one mutating owner, make a bounded change, validate, commit, and push `main`. Read-only review may run concurrently against the same exact head. Use a temporary branch/worktree only when rollback isolation, conflicting mutation, or exact-head validation genuinely requires it; remove its checkout in the same integration cycle after its unique state is retained by `main` or an intentional ref. Francis support data belongs under `D:\Francis-support`, never in permanent short-name drive-root worktrees. PRs should state the roadmap area touched, summarize validation actually run, include screenshots for UI changes, and call out any policy, shell, orb, HUD, or memory-contract risk explicitly.
