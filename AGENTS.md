...
Recent history uses Conventional Commit style with scopes, for example `feat(observability): surface watcher state age` and `fix(observability): pin francis observer to one session`. Branch from `origin/main` using the documented prefixes such as `codex/<slice-name>` or `hotfix/<slice-name>`. PRs should state the roadmap area touched, summarize validation actually run, include screenshots for UI changes, and call out any policy, shell, orb, HUD, or memory-contract risk explicitly.
