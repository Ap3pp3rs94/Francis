...
function SystemPanel(props: {
  ...
  const [telemetryFeedbackMemoryReadback, setTelemetryFeedbackMemoryReadback] =
  ...
  const [telemetryFeedbackMemoryAssistanceLoopSampleLoadedAt, setTelemetryFeedbackMemoryAssistanceLoopSampleLoadedAt] =
    useState<number | null>(null);
  const [telemetryFeedbackMemoryAssistanceAcceptanceAudit, setTelemetryFeedbackMemoryAssistanceAcceptanceAudit] =
    useState<TelemetryContextFeedbackMemoryAssistanceOperatorFeedbackLoopE2eAcceptanceAudit | null>(null);
  const [telemetryFeedbackMemoryAssistanceAcceptanceAuditError, setTelemetryFeedbackMemoryAssistanceAcceptanceAuditError] =
    useState<string | null>(null);
  const [
    telemetryFeedbackMemoryAssistanceAcceptanceAuditLoadedAt,
    setTelemetryFeedbackMemoryAssistanceAcceptanceAuditLoadedAt,
  ] = useState<number | null>(null);
  const [telemetryFeedbackMemoryAssistanceLiveSampleReadback, setTelemetryFeedbackMemoryAssistanceLiveSampleReadback] =
    useState<TelemetryContextFeedbackMemoryAssistanceOperatorFeedbackLoopLiveSampleReadback | null>(null);
  const [
    telemetryFeedbackMemoryAssistanceLiveSampleReadbackError,
    setTelemetryFeedbackMemoryAssistanceLiveSampleReadbackError,
  ] = useState<string | null>(null);
  const [
    telemetryFeedbackMemoryAssistanceLiveSampleReadbackLoadedAt,
    setTelemetryFeedbackMemoryAssistanceLiveSampleReadbackLoadedAt,
  ] = useState<number | null>(null);
  const [
    telemetryFeedbackMemoryAssistanceLiveSampleOperatorReview,
    setTelemetryFeedbackMemoryAssistanceLiveSampleOperatorReview,
  ] = useState<TelemetryContextFeedbackMemoryAssistanceOperatorFeedbackLoopLiveSampleOperatorReview | null>(null);
  const [
    telemetryFeedbackMemoryAssistanceLiveSampleOperatorReviewError,
    setTelemetryFeedbackMemoryAssistanceLiveSampleOperatorReviewError,
  ] = useState<string | null>(null);
  const [
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (nextTelemetryFeedbackMemoryReadback.status === "fulfilled") {
        setTelemetryFeedbackMemoryReadback(nextTelemetryFeedbackMemoryReadback.value);
        setTelemetryFeedbackMemoryReadbackError(null);
        setTelemetryFeedbackMemoryReadbackLoadedAt(refreshStartedAt);
      } else {
      ...
      if (nextTelemetryFeedbackMemoryAssistanceAcceptanceAudit.status === "fulfilled") {
        setTelemetryFeedbackMemoryAssistanceAcceptanceAudit(nextTelemetryFeedbackMemoryAssistanceAcceptanceAudit.value);
        setTelemetryFeedbackMemoryAssistanceAcceptanceAuditError(null);
        setTelemetryFeedbackMemoryAssistanceAcceptanceAuditLoadedAt(refreshStartedAt);
      } else {
        setTelemetryFeedbackMemoryAssistanceAcceptanceAuditError(
          telemetryError(nextTelemetryFeedbackMemoryAssistanceAcceptanceAudit.reason),
        );
        degradedFeeds.push("telemetry feedback memory assistance e2e acceptance audit");
      }

      if (nextTelemetryFeedbackMemoryAssistanceLiveSampleReadback.status === "fulfilled") {
        setTelemetryFeedbackMemoryAssistanceLiveSampleReadback(nextTelemetryFeedbackMemoryAssistanceLiveSampleReadback.value);
        setTelemetryFeedbackMemoryAssistanceLiveSampleReadbackError(null);
        setTelemetryFeedbackMemoryAssistanceLiveSampleReadbackLoadedAt(refreshStartedAt);
      } else {
        setTelemetryFeedbackMemoryAssistanceLiveSampleReadbackError(
          telemetryError(nextTelemetryFeedbackMemoryAssistanceLiveSampleReadback.reason),
        );
        degradedFeeds.push("telemetry feedback memory assistance live sample readback");
      }

      if (nextTelemetryFeedbackMemoryAssistanceLiveSampleOperatorReview.status === "fulfilled") {
        setTelemetryFeedbackMemoryAssistanceLiveSampleOperatorReview(
          nextTelemetryFeedbackMemoryAssistanceLiveSampleOperatorReview.value,
        );
        setTelemetryFeedbackMemoryAssistanceLiveSampleOperatorReviewError(null);
        setTelemetryFeedbackMemoryAssistanceLiveSampleOperatorReviewLoadedAt(refreshStartedAt);
      } else {
        setTelemetryFeedbackMemoryAssistanceLiveSampleOperatorReviewError(
  ...
  const telemetryFeedbackMemoryReadbackStatus =
    safeString(telemetryFeedbackMemoryReadback?.status).trim() ||
    (telemetryFeedbackMemoryReadbackError ? "unavailable" : "unloaded");
  const telemetryFeedbackMemoryReadbackTone = telemetryFeedbackMemoryReadbackError
    ? "blocked"
    : telemetryFeedbackMemoryReadback
      ? telemetryFeedbackMemoryReadback.count > 0
        ? "running"
        : "dormant"
      : "standby";
  const telemetryFeedbackMemoryReadbackDetail = telemetryFeedbackMemoryReadbackError
    ? `Feedback memory retrieval readback could not refresh: ${telemetryFeedbackMemoryReadbackError}`
    : telemetryFeedbackMemoryReadback
  ...
  const telemetryFeedbackMemoryAssistanceAcceptanceAuditStatus =
    safeString(telemetryFeedbackMemoryAssistanceAcceptanceAudit?.status).trim() ||
    (telemetryFeedbackMemoryAssistanceAcceptanceAuditError ? "unavailable" : "unloaded");
  const telemetryFeedbackMemoryAssistanceAcceptanceAuditTone = telemetryFeedbackMemoryAssistanceAcceptanceAuditError
    ? "blocked"
    : telemetryFeedbackMemoryAssistanceAcceptanceAudit
      ? telemetryFeedbackMemoryAssistanceAcceptanceAudit.acceptance_ready
        ? "running"
        : "dormant"
      : "standby";
  const telemetryFeedbackMemoryAssistanceAcceptanceAuditSample =
    telemetryFeedbackMemoryAssistanceAcceptanceAudit?.sample ?? {};
  const telemetryFeedbackMemoryAssistanceAcceptanceAuditDetail = telemetryFeedbackMemoryAssistanceAcceptanceAuditError
    ? `Feedback memory assistance e2e acceptance audit could not refresh: ${telemetryFeedbackMemoryAssistanceAcceptanceAuditError}`
    : telemetryFeedbackMemoryAssistanceAcceptanceAudit
      ? `Feedback memory assistance acceptance ${telemetryFeedbackMemoryAssistanceAcceptanceAudit.status}; criteria ${telemetryFeedbackMemoryAssistanceAcceptanceAudit.ready_count}/${telemetryFeedbackMemoryAssistanceAcceptanceAudit.required_count}, sample ${safeString(telemetryFeedbackMemoryAssistanceAcceptanceAuditSample.status).trim() || "unknown"}.`
      : "Feedback memory assistance e2e acceptance audit has not loaded yet.";
  const telemetryFeedbackMemoryAssistanceLiveSampleStatus =
    safeString(telemetryFeedbackMemoryAssistanceLiveSampleReadback?.status).trim() ||
    (telemetryFeedbackMemoryAssistanceLiveSampleReadbackError ? "unavailable" : "unloaded");
  const telemetryFeedbackMemoryAssistanceLiveSampleTone = telemetryFeedbackMemoryAssistanceLiveSampleReadbackError
    ? "blocked"
    : telemetryFeedbackMemoryAssistanceLiveSampleReadback
      ? telemetryFeedbackMemoryAssistanceLiveSampleReadback.live_sample_observed
        ? "running"
        : "dormant"
      : "standby";
  const telemetryFeedbackMemoryAssistanceLiveSampleChat = telemetryFeedbackMemoryAssistanceLiveSampleReadback?.chat ?? {};
  const telemetryFeedbackMemoryAssistanceLiveSampleMemory =
    telemetryFeedbackMemoryAssistanceLiveSampleReadback?.memory ?? {};
  const telemetryFeedbackMemoryAssistanceLiveSampleDetail =
    telemetryFeedbackMemoryAssistanceLiveSampleReadbackError
      ? `Feedback memory assistance live sample readback could not refresh: ${telemetryFeedbackMemoryAssistanceLiveSampleReadbackError}`
      : telemetryFeedbackMemoryAssistanceLiveSampleReadback
        ? `Feedback memory assistance live sample ${telemetryFeedbackMemoryAssistanceLiveSampleReadback.status}; criteria ${telemetryFeedbackMemoryAssistanceLiveSampleReadback.ready_count}/${telemetryFeedbackMemoryAssistanceLiveSampleReadback.required_count}, chat ${safeString(telemetryFeedbackMemoryAssistanceLiveSampleChat.status).trim() || "missing"}.`
        : "Feedback memory assistance live sample readback has not loaded yet.";
  const telemetryFeedbackMemoryAssistanceLiveSampleOperatorReviewStatus =
    safeString(telemetryFeedbackMemoryAssistanceLiveSampleOperatorReview?.status).trim() ||
    (telemetryFeedbackMemoryAssistanceLiveSampleOperatorReviewError ? "unavailable" : "unloaded");
  const telemetryFeedbackMemoryAssistanceLiveSampleOperatorReviewTone =
    telemetryFeedbackMemoryAssistanceLiveSampleOperatorReviewError
      ? "blocked"
      : telemetryFeedbackMemoryAssistanceLiveSampleOperatorReview
        ? telemetryFeedbackMemoryAssistanceLiveSampleOperatorReview.operator_review_ready
          ...
          : "dormant"
        : "standby";
  const telemetryFeedbackMemoryAssistanceLiveSampleOperatorDecision =
    telemetryFeedbackMemoryAssistanceLiveSampleOperatorReview?.operator_decision ?? {};
  const telemetryFeedbackMemoryAssistanceLiveSampleOperatorDecisionRequired = Boolean(
    telemetryFeedbackMemoryAssistanceLiveSampleOperatorDecision.required,
  );
  const telemetryFeedbackMemoryAssistanceLiveSampleOperatorDecisionRecorded = Boolean(
    telemetryFeedbackMemoryAssistanceLiveSampleOperatorDecision.recorded,
  );
  const telemetryFeedbackMemoryAssistanceLiveSampleOperatorReviewDetail =
    telemetryFeedbackMemoryAssistanceLiveSampleOperatorReviewError
      ? `Feedback memory assistance live sample operator review could not refresh: ${telemetryFeedbackMemoryAssistanceLiveSampleOperatorReviewError}`
      : telemetryFeedbackMemoryAssistanceLiveSampleOperatorReview
        ...
        >
          ...
          >
            ...
            {lensStage6NextHandoff.currentHandoff ? (
              <div style={{ fontSize: 11, color: THEME.muted, marginTop: 4 }}>
                current handoff <code>{lensStage6NextHandoff.currentHandoff}</code>
              </div>
            ) : null}
            {lensStage6NextHandoff.currentProof ? (
              <div style={{ fontSize: 11, color: THEME.muted, marginTop: 4 }}>
                proof <code>{lensStage6NextHandoff.currentProof}</code>
              </div>
            ) : null}
            ...
            {lensStage6NextHandoff.prerequisiteSource ? (
              <div style={{ fontSize: 11, color: "#cce7e2", marginTop: 7 }}>
                ...
                {lensStage6NextHandoff.prerequisiteHandoff ? (
                  <>
                    {" / "}handoff <code>{lensStage6NextHandoff.prerequisiteHandoff}</code>
                  </>
                ) : null}
                {lensStage6NextHandoff.prerequisiteProof ? (
                  <>
            ...
            {lensStage6NextHandoff.firstMissingSource ? (
              <div style={{ fontSize: 11, color: "#cce7e2", marginTop: 4 }}>
                first missing <code>{lensStage6NextHandoff.firstMissingSource}</code>
                {lensStage6NextHandoff.firstMissingHandoff ? (
                  <>
                    {" / "}handoff <code>{lensStage6NextHandoff.firstMissingHandoff}</code>
                  </>
                ) : null}
                ...
                {lensStage6NextHandoff.firstMissingAuthority ? (
                  <>
                    {" / "}authority <code>{lensStage6NextHandoff.firstMissingAuthority}</code>
                  </>
                ) : null}
              </div>
            ) : null}
            {!lensStage6NextHandoff.loaded ? (
      ...
      <div id="francis-telemetry-status" style={{ ...summaryCardStyle(), marginTop: 12 }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            ...
            <span style={badgeStyle(telemetryFeedbackMemoryAssistanceAcceptanceAuditTone)}>
              Assist acceptance {telemetryFeedbackMemoryAssistanceAcceptanceAuditStatus}
            </span>
            <span style={badgeStyle(telemetryFeedbackMemoryAssistanceLiveSampleTone)}>
              Live sample {telemetryFeedbackMemoryAssistanceLiveSampleStatus}
            </span>
        ...
        <div
          style={{
            fontSize: 12,
            color: telemetryFeedbackMemoryAssistanceAcceptanceAuditError ? "#ffcf9d" : THEME.text,
            marginTop: 8,
          }}
        >
          {telemetryFeedbackMemoryAssistanceAcceptanceAuditDetail}
        </div>
        <div
          style={{
            fontSize: 12,
            color: telemetryFeedbackMemoryAssistanceLiveSampleReadbackError ? "#ffcf9d" : THEME.text,
            marginTop: 8,
          }}
        >
          {telemetryFeedbackMemoryAssistanceLiveSampleDetail}
        </div>
        <div
          style={{
            fontSize: 12,
            color: telemetryFeedbackMemoryAssistanceLiveSampleOperatorReviewError ? "#ffcf9d" : THEME.text,
            marginTop: 8,
          }}
        >
        ...
        {telemetryFeedbackMemoryAssistanceAcceptanceAudit ? (
          ...
          >
            <div style={{ border: `1px solid ${THEME.panelBorder}`, borderRadius: 10, padding: 10, background: "#121212" }}>
              <div style={{ fontSize: 11, color: THEME.muted }}>E2e acceptance</div>
              <div style={{ fontSize: 18, fontWeight: 700, marginTop: 4 }}>
                {telemetryFeedbackMemoryAssistanceAcceptanceAudit.acceptance_ready ? "ready" : "waiting"}
              </div>
              <div style={{ fontSize: 11, color: THEME.muted, marginTop: 4 }}>
                criteria <code>{telemetryFeedbackMemoryAssistanceAcceptanceAudit.ready_count}</code>/
                <code>{telemetryFeedbackMemoryAssistanceAcceptanceAudit.required_count}</code>
                {" / "}sample{" "}
                <code>{safeString(telemetryFeedbackMemoryAssistanceAcceptanceAuditSample.status).trim() || "unknown"}</code>
              </div>
              {telemetryFeedbackMemoryAssistanceAcceptanceAuditLoadedAt ? (
                <div style={{ fontSize: 11, color: THEME.muted, marginTop: 4 }}>
                  Loaded {toLocaleTime(telemetryFeedbackMemoryAssistanceAcceptanceAuditLoadedAt)}
                </div>
              ) : null}
            </div>
            <div style={{ border: `1px solid ${THEME.panelBorder}`, borderRadius: 10, padding: 10, background: "#121212" }}>
              <div style={{ fontSize: 11, color: THEME.muted }}>Acceptance guards</div>
              <div style={{ fontSize: 12, color: THEME.text, marginTop: 6 }}>
                sends <code>{telemetryFeedbackMemoryAssistanceAcceptanceAudit.sends_chat ? "true" : "false"}</code>
                {" / "}feedback{" "}
                <code>{telemetryFeedbackMemoryAssistanceAcceptanceAudit.writes_feedback ? "true" : "false"}</code>
              </div>
              <div style={{ fontSize: 12, color: THEME.text, marginTop: 4 }}>
                memory <code>{telemetryFeedbackMemoryAssistanceAcceptanceAudit.writes_memory ? "true" : "false"}</code>
                {" / "}model <code>{telemetryFeedbackMemoryAssistanceAcceptanceAudit.calls_model ? "true" : "false"}</code>
              </div>
              <div style={{ fontSize: 11, color: THEME.muted, marginTop: 4 }}>
                next <code>{telemetryFeedbackMemoryAssistanceAcceptanceAudit.next_smallest_truthful_gap || "live_sample_run"}</code>
              </div>
            </div>
          </div>
        ) : null}
        {telemetryFeedbackMemoryAssistanceLiveSampleReadback ? (
          ...
          >
            <div style={{ border: `1px solid ${THEME.panelBorder}`, borderRadius: 10, padding: 10, background: "#121212" }}>
              <div style={{ fontSize: 11, color: THEME.muted }}>Live sample</div>
              <div style={{ fontSize: 18, fontWeight: 700, marginTop: 4 }}>
                {telemetryFeedbackMemoryAssistanceLiveSampleReadback.live_sample_observed ? "observed" : "waiting"}
              </div>
              <div style={{ fontSize: 11, color: THEME.muted, marginTop: 4 }}>
                criteria <code>{telemetryFeedbackMemoryAssistanceLiveSampleReadback.ready_count}</code>/
                <code>{telemetryFeedbackMemoryAssistanceLiveSampleReadback.required_count}</code>
                {" / "}chat <code>{safeString(telemetryFeedbackMemoryAssistanceLiveSampleChat.status).trim() || "missing"}</code>
              </div>
              {telemetryFeedbackMemoryAssistanceLiveSampleReadbackLoadedAt ? (
                <div style={{ fontSize: 11, color: THEME.muted, marginTop: 4 }}>
                  Loaded {toLocaleTime(telemetryFeedbackMemoryAssistanceLiveSampleReadbackLoadedAt)}
                </div>
              ) : null}
            </div>
            <div style={{ border: `1px solid ${THEME.panelBorder}`, borderRadius: 10, padding: 10, background: "#121212" }}>
              <div style={{ fontSize: 11, color: THEME.muted }}>Live evidence</div>
              <div style={{ fontSize: 12, color: THEME.text, marginTop: 6 }}>
                ledger{" "}
                <code>{telemetryFeedbackMemoryAssistanceLiveSampleReadback.reads_conversation_ledger ? "read" : "none"}</code>
                {" / "}feedback <code>{telemetryFeedbackMemoryAssistanceLiveSampleReadback.reads_feedback ? "read" : "none"}</code>
              </div>
              <div style={{ fontSize: 12, color: THEME.text, marginTop: 4 }}>
                memory <code>{safeString(telemetryFeedbackMemoryAssistanceLiveSampleMemory.event_id).trim() || "missing"}</code>
              </div>
              <div style={{ fontSize: 11, color: THEME.muted, marginTop: 4 }}>
                next <code>{telemetryFeedbackMemoryAssistanceLiveSampleReadback.next_smallest_truthful_gap || "live_sample_run"}</code>
              </div>
            </div>
            <div style={{ border: `1px solid ${THEME.panelBorder}`, borderRadius: 10, padding: 10, background: "#121212" }}>
              <div style={{ fontSize: 11, color: THEME.muted }}>Readback guards</div>
              <div style={{ fontSize: 12, color: THEME.text, marginTop: 6 }}>
                sends <code>{telemetryFeedbackMemoryAssistanceLiveSampleReadback.sends_chat ? "true" : "false"}</code>
                {" / "}feedback{" "}
                <code>{telemetryFeedbackMemoryAssistanceLiveSampleReadback.writes_feedback ? "true" : "false"}</code>
              </div>
              <div style={{ fontSize: 12, color: THEME.text, marginTop: 4 }}>
                memory <code>{telemetryFeedbackMemoryAssistanceLiveSampleReadback.writes_memory ? "true" : "false"}</code>
                {" / "}model <code>{telemetryFeedbackMemoryAssistanceLiveSampleReadback.calls_model ? "true" : "false"}</code>
              </div>
            </div>
          </div>
        ) : null}
        {telemetryFeedbackMemoryAssistanceLiveSampleOperatorReview ? (
          ...
          >
            <div style={{ border: `1px solid ${THEME.panelBorder}`, borderRadius: 10, padding: 10, background: "#121212" }}>
              <div style={{ fontSize: 11, color: THEME.muted }}>Live sample review</div>
              <div style={{ fontSize: 18, fontWeight: 700, marginTop: 4 }}>
                {telemetryFeedbackMemoryAssistanceLiveSampleOperatorReview.operator_review_ready ? "ready" : "waiting"}
              </div>
              <div style={{ fontSize: 11, color: THEME.muted, marginTop: 4 }}>
                criteria <code>{telemetryFeedbackMemoryAssistanceLiveSampleOperatorReview.ready_count}</code>/
                <code>{telemetryFeedbackMemoryAssistanceLiveSampleOperatorReview.required_count}</code>
                {" / "}sample{" "}
                <code>
                  {telemetryFeedbackMemoryAssistanceLiveSampleOperatorReview.live_sample_observed ? "observed" : "missing"}
                </code>
              </div>
              {telemetryFeedbackMemoryAssistanceLiveSampleOperatorReviewLoadedAt ? (
                <div style={{ fontSize: 11, color: THEME.muted, marginTop: 4 }}>
                  Loaded {toLocaleTime(telemetryFeedbackMemoryAssistanceLiveSampleOperatorReviewLoadedAt)}
                </div>
              ) : null}
            </div>
            <div style={{ border: `1px solid ${THEME.panelBorder}`, borderRadius: 10, padding: 10, background: "#121212" }}>
              <div style={{ fontSize: 11, color: THEME.muted }}>Operator decision</div>
              <div style={{ fontSize: 12, color: THEME.text, marginTop: 6 }}>
                required <code>{telemetryFeedbackMemoryAssistanceLiveSampleOperatorDecisionRequired ? "true" : "false"}</code>
                {" / "}recorded{" "}
                <code>{telemetryFeedbackMemoryAssistanceLiveSampleOperatorDecisionRecorded ? "true" : "false"}</code>
              </div>
              <div style={{ fontSize: 11, color: THEME.muted, marginTop: 4 }}>
                next{" "}
                <code>
                  {telemetryFeedbackMemoryAssistanceLiveSampleOperatorReview.next_smallest_truthful_gap ||
                    "live_sample_operator_review"}
                </code>
              </div>
            </div>
            <div style={{ border: `1px solid ${THEME.panelBorder}`, borderRadius: 10, padding: 10, background: "#121212" }}>
              <div style={{ fontSize: 11, color: THEME.muted }}>Review guards</div>
              <div style={{ fontSize: 12, color: THEME.text, marginTop: 6 }}>
                sends <code>{telemetryFeedbackMemoryAssistanceLiveSampleOperatorReview.sends_chat ? "true" : "false"}</code>
                {" / "}feedback{" "}
                <code>{telemetryFeedbackMemoryAssistanceLiveSampleOperatorReview.writes_feedback ? "true" : "false"}</code>
              </div>
              <div style={{ fontSize: 12, color: THEME.text, marginTop: 4 }}>
        ...
        {telemetryFeedbackMemoryReadback ? (
          ...
          >
            <div style={{ border: `1px solid ${THEME.panelBorder}`, borderRadius: 10, padding: 10, background: "#121212" }}>
              <div style={{ fontSize: 11, color: THEME.muted }}>Memory readback</div>
              <div style={{ fontSize: 18, fontWeight: 700, marginTop: 4 }}>
                {telemetryFeedbackMemoryReadback.count}/{telemetryFeedbackMemoryReadback.total}
              </div>
              <div style={{ fontSize: 11, color: THEME.muted, marginTop: 4 }}>
                skipped <code>{telemetryFeedbackMemoryReadback.skipped_count}</code>
              </div>
              {telemetryFeedbackMemoryReadbackLoadedAt ? (
                <div style={{ fontSize: 11, color: THEME.muted, marginTop: 4 }}>
                  Loaded {toLocaleTime(telemetryFeedbackMemoryReadbackLoadedAt)}
                </div>
              ) : null}
            </div>
            <div style={{ border: `1px solid ${THEME.panelBorder}`, borderRadius: 10, padding: 10, background: "#121212" }}>
              <div style={{ fontSize: 11, color: THEME.muted }}>Memory governance</div>
              <div style={{ fontSize: 11, color: THEME.muted, marginTop: 6 }}>
                reads <code>{telemetryFeedbackMemoryReadback.reads_memory ? "true" : "false"}</code>
                {" / "}writes <code>{telemetryFeedbackMemoryReadback.writes_memory ? "true" : "false"}</code>
              </div>
              <div style={{ fontSize: 11, color: THEME.muted, marginTop: 4 }}>
                trains <code>{telemetryFeedbackMemoryReadback.trains_model ? "true" : "false"}</code>
                {" / "}exec <code>{telemetryFeedbackMemoryReadback.grants_execution_authority ? "true" : "false"}</code>
              </div>
            </div>
            <div style={{ border: `1px solid ${THEME.panelBorder}`, borderRadius: 10, padding: 10, background: "#121212" }}>
              <div style={{ fontSize: 11, color: THEME.muted }}>Latest memory signal</div>
              {latestTelemetryFeedbackMemoryEvent ? (
                <>
                  <div style={{ fontSize: 12, color: THEME.text, marginTop: 6 }}>
                    {latestTelemetryFeedbackMemoryEvent.kind || "memory event"}
                  </div>
                  <div style={{ fontSize: 11, color: THEME.muted, marginTop: 4 }}>
                    policy <code>{safeString(latestTelemetryFeedbackMemoryEvent.retention.policy).trim() || "unknown"}</code>
                  </div>
                </>
              ) : (
                <div style={{ fontSize: 12, color: THEME.muted, marginTop: 6 }}>
