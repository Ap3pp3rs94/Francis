...
function parseMissionMemoryReceipt(raw: unknown): MissionMemoryReceipt | null {
  ...
  return hasReceiptIdentity ? receipt : null;
}
...
function summarizeDeadletterApprovalReplacement(item: MissionDeadletterLike): string | undefined {
  const kind = safeString(item.last_task_approval_replacement_kind, "").trim();
...
function deadletterApprovalReviewLabel(item: MissionDeadletterLike): string {
  ...
  const reason = safeString(item.last_task_approval_replacement_reason, "").trim();
  const changedKeys = safeStringArray(item.last_task_approval_replacement_changed_keys);
  return kind || reason || changedKeys.length > 0 ? "Review replacement approval" : "Review approval";
}
    ...
    .map((item, index) => {
      ...
      return {
        item: {
          ...
          last_task_approval_replacement_kind:
            safeString(item.last_task_approval_replacement_kind, "").trim() || undefined,
...
function parseWorldStateMissionRecoveryProjection(raw: unknown): WorldStateMissionRecoveryProjection | undefined {
  ...
  const projection: WorldStateMissionRecoveryProjection = {
    ...
    replacement_mission_id: safeString(raw["replacement_mission_id"], ""),
    replacement_status: safeString(raw["replacement_status"], ""),
    replacement_objective: safeString(raw["replacement_objective"], ""),
    replacement_next_step: safeString(raw["replacement_next_step"], ""),
    replacement_last_task_id: safeString(raw["replacement_last_task_id"], ""),
    replacement_last_task_status: safeString(raw["replacement_last_task_status"], ""),
    replacement_updated_at: safeString(raw["replacement_updated_at"], ""),
    replacement_terminal: safeBoolean(raw["replacement_terminal"], false),
    replacement_error: safeString(raw["replacement_error"], ""),
  };
  if (
...
function parseContinuityFocusItem(raw: unknown): ContinuityBriefingFocusItem | null {
  ...
  const item: ContinuityBriefingFocusItem = {
    ...
    last_task_approval_replacement_kind: safeString(raw["last_task_approval_replacement_kind"], ""),
