...
def test_fr017_pilot_movement_initializer_status_preflights_without_writing(
    tmp_path: Path,
) -> None:
    measurement_path, mockup_path, mannequin_path, static_fit_path = _write_ready_upstream(tmp_path)
    movement_path = tmp_path / "candidate-movement.json"
    ...
    assert result["status"] == "pilot_movement_record_initializer_status"
    ...
    assert result["operator_supplied_pilot_movement_input_recorded"] is False
    ...
    assert "fr017-new-pilot-movement-record.ps1 -Mode Create" in result["create_command_template"]
    assert "fr017-pilot-movement-gate.ps1 -Mode Status" in result["pilot_movement_status_command_template"]
