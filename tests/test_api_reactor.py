...
def test_reactor_event_routes_enqueue_and_readback(monkeypatch, tmp_path: Path) -> None:
    ...
    assert empty_status.json()["dispatch_engine_boundary_actions"] == ["dispatch", "execute", "mutate", "plugin_run"]
...
def test_reactor_plugin_run_dispatch_route_blocks_without_execution(
    ...
    queued = client.post(
        ...
        json={
            ...
            "summary": "Plugin run should block at the Reactor boundary.",
            "actor": _REACTOR_ACTOR,
            "mode": "pilot",
            "action_class": "plugin_run",
            "max_actions": 1,
            "metadata": {"plugin_id": "plugin.api.boundary"},
        },
    )
    assert queued.status_code == 200
    ...
    attempted = client.post(
        ...
        json={
            "event_id": event_id,
            "actor": _REACTOR_ACTOR,
            "reason": "prove API plugin run boundary without plugin execution",
        },
    )
    ...
    assert execution["plugin_id"] == "plugin.api.boundary"
    ...
    assert verification["verification_outcome"] == "plugin_run_dispatch_not_enabled"
    ...
    assert stable_return["stable_state"] == "plugin_run_dispatch_not_enabled"
    ...
    assert review_body["items"][0]["review"]["gate"] == "plugin_run_dispatch_not_enabled"
    ...
    assert status_body["dispatch_engine_boundary_actions"] == ["dispatch", "execute", "mutate", "plugin_run"]
...
def test_reactor_dispatch_action_route_blocks_without_execution(
    monkeypatch,
    tmp_path: Path,
) -> None:
    data_root = tmp_path / "francis_data"
    ...
    queued = client.post(
        ...
        json={
            "trigger_source": "user_request",
            "summary": "Dispatch should block at the Reactor boundary.",
            "actor": _REACTOR_ACTOR,
            "mode": "pilot",
    ...
    attempted = client.post(
        ...
        json={
            "event_id": event_id,
            "actor": _REACTOR_ACTOR,
            "reason": "prove API dispatch boundary without dispatch authority",
        },
    )
    assert attempted.status_code == 200
    body = attempted.json()
    ...
    event = body["event"]
    assert event["status"] == "dispatch_blocked"
    assert event["stable_state"] == "dispatch_action_not_enabled"
    assert event["dispatch"]["engine"] == "dispatch"
    assert event["dispatch"]["applied"] is False
    assert event["dispatch"]["execution_started"] is False
    assert event["dispatch"]["blocked_route"] == "operator_review"

    execution = event["latest_dispatch_execution_receipt"]
    assert execution["route"] == "dispatch"
    assert execution["status"] == "blocked"
    assert execution["outcome"] == "dispatch_action_not_enabled"
    assert execution["execution_started"] is False
    assert execution["dispatch_applied"] is False
    assert execution["readback_only"] is True
    assert execution["governance"]["dispatch_action_authority"] is False
    assert execution["governance"]["execution_authority"] is False
    assert execution["governance"]["dispatch_authority"] is False
    ...
    assert verification["route"] == "operator_review"
    assert verification["verification_status"] == "not_run"
    assert verification["verification_outcome"] == "dispatch_action_not_enabled"
    assert verification["execution_started"] is False
    assert verification["dispatch_applied"] is False
    stable_return = event["latest_stable_return"]
    assert stable_return["route"] == "operator_review"
    assert stable_return["stable_state"] == "dispatch_action_not_enabled"
    assert stable_return["source_receipt_kind"] == "reactor.dispatch.execution.receipt"
    assert stable_return["execution_started"] is False
    assert stable_return["dispatch_applied"] is False
    ...
    assert review_body["available_total"] == 1
    assert review_body["items"][0]["event_id"] == event_id
    assert review_body["items"][0]["review"]["gate"] == "dispatch_action_not_enabled"
    assert review_body["items"][0]["review"]["receipt_kind"] == "reactor.dispatch_blocker"
    ...
    assert status_body["dispatch_engine_boundary_actions"] == ["dispatch", "execute", "mutate", "plugin_run"]
    assert status_body["status_counts"] == {"dispatch_blocked": 1}
    assert status_body["stable_state_counts"] == {"dispatch_action_not_enabled": 1}
    assert status_body["dispatch_execution_counts"] == {"blocked": 1}
    assert status_body["verification_counts"] == {"not_run": 1}
    assert status_body["verification_outcome_counts"] == {"dispatch_action_not_enabled": 1}
    assert status_body["governance"]["execution_authority"] is False
    assert status_body["governance"]["dispatch_authority"] is False
    ...
    for trigger_source, trigger_type, outcome in cases:
        queued = client.post(
            ...
            json={
                ...
                "summary": f"{trigger_source} needs bounded Reactor classification",
                "actor": _REACTOR_ACTOR,
                "mode": "assist",
                "risk_tier": "normal",
                "max_actions": 1,
                "metadata": {"surface": trigger_source},
            },
        )
        assert queued.status_code == 200
        event_id = str(queued.json()["event_id"])
        ...
        dispatched = client.post(
            ...
            json={
                "event_id": event_id,
                "actor": _REACTOR_ACTOR,
                "reason": f"classify {trigger_source} without execution",
            },
        )
        assert dispatched.status_code == 200
