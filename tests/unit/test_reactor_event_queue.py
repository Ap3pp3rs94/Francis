...
def test_reactor_dispatch_engine_blocks_plugin_run_boundary_without_execution(
    ...
    status = reactor_status()
    assert status["dispatch_engine_boundary_actions"] == ["dispatch", "execute", "mutate", "plugin_run"]
...
def test_reactor_dispatch_engine_blocks_dispatch_boundary_without_execution(
    monkeypatch,
    tmp_path: Path,
) -> None:
    data_root = tmp_path / "francis_data"
    ...
    created = enqueue_event(
        {
            "trigger_source": "user_request",
            "summary": "Generic dispatch needs a governed Reactor boundary.",
            "mode": "pilot",
    ...
    attempted = record_dispatch_attempt(
        ...
        {
            "actor": "reactor.test",
            "reason": "prove generic dispatch stays blocked until a governed dispatcher exists",
        },
    )
    ...
    assert execution["gate"] == "reactor_dispatch_boundary"
    ...
    _assert_verification(
        ...
        source_kind="reactor.dispatch.execution.receipt",
        verification_status="not_run",
        verification_outcome="dispatch_action_not_enabled",
    )
    _assert_stable_return(
        event,
        route="operator_review",
    ...
    assert review_queue["available_total"] == 1
    review = review_queue["items"][0]["review"]
    assert review["gate"] == "dispatch_action_not_enabled"
    assert review["receipt_kind"] == "reactor.dispatch_blocker"
    assert review["execution_started"] is False
    assert review["applied"] is False
    assert {item["event_id"] for item in list_events(receipt_kind="reactor.dispatch.execution.receipt")} == {event_id}
    ...
    assert status["dispatch_engine_boundary_actions"] == ["dispatch", "execute", "mutate", "plugin_run"]
    assert status["status_counts"] == {"dispatch_blocked": 1}
    assert status["stable_state_counts"] == {"dispatch_action_not_enabled": 1}
    assert status["dispatch_execution_counts"] == {"blocked": 1}
    assert status["verification_counts"] == {"not_run": 1}
    assert status["verification_outcome_counts"] == {"dispatch_action_not_enabled": 1}
...
def test_reactor_dispatch_engine_classifies_remaining_default_triggers_without_generic_deferment(
    monkeypatch,
    tmp_path: Path,
) -> None:
    data_root = tmp_path / "francis_data"
