...
def test_lens_stage6_completion_audit_accepts_consumed_authority_handoff_evidence() -> None:
    ...
    assert "$Stage6ClosureReadbackObserved = (" in script
    assert "stage6_closure_readback = $Stage6ClosureReadbackObserved" in script
    assert "$Stage6CompletionEvidenceReviewed = $Stage6ClosureReadbackObserved -or" in script
    assert "stage6_prerequisite_bringup_plan = ($Stage6ClosureReadbackObserved -or" in script
...
def test_lens_stage6_completion_audit_consumes_tray_presence_blocker_readback() -> None:
    ...
    assert (
        "[string]$SummonTrayPresenceBlockerProof.next_smallest_truthful_gap -eq 'summon_overlay_window_blocker_boundary'"
        in script
    )
    assert "[bool]$SummonTrayPresenceBlockerProof.tray_presence_boundary_observed" in script
    assert "[bool]$SummonTrayPresenceBlockerProofGovernance.read_only_contract" in script
    assert (
        "-not $SummonAnywhereBlockersProofFirstFamilyTrayObserved -or $SummonTrayPresenceBlockerProofObserved" in script
    )
    assert "[string]$SummonTrayPresenceBlockerProof.next_smallest_truthful_gap" in script
...
def test_lens_stage6_completion_audit_budgets_transition_plan_wrapper() -> None:
    ...
    assert "$Payload | ConvertTo-Json -Depth 12" in script
...
def test_lens_stage6_completion_audit_consumes_stage6_prerequisite_bringup_plan_readback() -> None:
    ...
    assert "$Stage6PrerequisiteBringupAppliedEnablementReceiptReviewConsumed = (" in script
    assert "$Stage6AppliedEnablementResidentClaimBoundaryReadbackObserved\n)" in script
    ...
    assert "-not $Stage6PrerequisiteBringupAppliedEnablementReceiptReviewConsumed" in receipt_review_pending_block
    ...
    assert (
        "applied_enablement_receipt_review_consumed = "
        "$Stage6PrerequisiteBringupAppliedEnablementReceiptReviewConsumed" in script
    )
    assert (
...
def test_lens_stage6_completion_audit_preserves_resident_runtime_service_control_authority_readback() -> None:
    script = (_repo_root() / "scripts" / "lens-stage6-completion-audit.ps1").read_text(encoding="utf-8")

    assert (
        "[string]$ResidentRuntimeBoundary.service_control_authority_required -eq 'service_control_authority'"
    ) in script
    assert (
...
def test_lens_stage6_completion_audit_preserves_resident_runtime_overlay_control_authority_readback() -> None:
    script = (_repo_root() / "scripts" / "lens-stage6-completion-audit.ps1").read_text(encoding="utf-8")

    assert (
        "[string]$ResidentRuntimeBoundary.overlay_control_authority_required -eq 'overlay_control_authority'" in script
    )
    assert (
...
def test_lens_stage6_completion_audit_observes_resident_runtime_process_supervision_authority_gate() -> None:
    script = (_repo_root() / "scripts" / "lens-stage6-completion-audit.ps1").read_text(encoding="utf-8")

    required_gate = (
        "[string]$ResidentRuntimeProcessSupervisionBoundaryProof.authority_required -eq 'process_supervision_authority'"
    )
    granted_gate = "-not [bool]$ResidentRuntimeProcessSupervisionBoundaryProof.authority_granted"
...
def test_lens_stage6_completion_audit_observes_resident_runtime_tray_presence_authority_gate() -> None:
    script = (_repo_root() / "scripts" / "lens-stage6-completion-audit.ps1").read_text(encoding="utf-8")

    required_gate = "[string]$ResidentRuntimeTrayPresenceBoundaryProof.authority_required -eq 'tray_presence_authority'"
    granted_gate = "-not [bool]$ResidentRuntimeTrayPresenceBoundaryProof.authority_granted"
...
def test_lens_stage6_completion_audit_observes_resident_runtime_resident_claim_authority_gate() -> None:
    script = (_repo_root() / "scripts" / "lens-stage6-completion-audit.ps1").read_text(encoding="utf-8")

    required_gate = (
        "[string]$ResidentRuntimeResidentClaimBoundaryProof.authority_required -eq 'resident_claim_authority'"
    )
    granted_gate = "-not [bool]$ResidentRuntimeResidentClaimBoundaryProof.authority_granted"
...
def test_lens_stage6_completion_audit_preserves_process_supervision_handoff_authority_readback() -> None:
    ...
    assert (
        "authority_required = [string]$ResidentHostRuntimeBoundaryProofRecommendedHandoff.authority_required" in script
    )
    assert "authority_granted = [bool]$ResidentHostRuntimeBoundaryProofRecommendedHandoff.authority_granted" in script
...
def test_lens_stage6_completion_audit_observes_resident_host_runtime_authority_gate() -> None:
    script = (_repo_root() / "scripts" / "lens-stage6-completion-audit.ps1").read_text(encoding="utf-8")

    assert "[string]$ResidentHostRuntimeBoundaryProof.authority_required -eq 'process_supervision_authority'" in script
    assert "-not [bool]$ResidentHostRuntimeBoundaryProof.authority_granted" in script
    assert (
        "[string]$ResidentHostRuntimeBoundaryProofRecommendedHandoff.authority_required -eq "
...
def test_lens_stage6_completion_audit_observes_summon_family_chain_authority_gates() -> None:
    script = (_repo_root() / "scripts" / "lens-stage6-completion-audit.ps1").read_text(encoding="utf-8")
    ...
    assert "$SummonAnywhereBlockersProofDelegatedAuthorityBoundaryObserved = (" in script
    assert "@($SummonAnywhereBlockersProofAuthorityBlockers).Count -eq 0" in script
    assert "$SummonAnywhereBlockersProofDelegatedAuthorityBoundaryObserved" in script
    ...
    assert (
        "$SummonAnywhereBlockersProofGlobalHotkeyBlockers -contains 'hotkey_registration_authority_not_granted' -or"
        in script
    )
    assert (
...
def test_lens_stage6_completion_audit_accepts_current_checkpoint_summon_family_handoff() -> None:
    script = (_repo_root() / "scripts" / "lens-stage6-completion-audit.ps1").read_text(encoding="utf-8")

    assert "$CheckpointSummonEnablementGateLegacyResidentFirstFamilyHandoffObserved = (" in script
    assert "$CheckpointSummonEnablementGateDelegatedResidentFirstFamilyHandoffObserved = (" in script
    assert "$CheckpointSummonEnablementGateCurrentFirstFamilyHandoffObserved = (" in script
    assert "$CheckpointSummonEnablementGateFirstFamilyHandoffObserved = (" in script
    assert "$CheckpointSummonEnablementGateFirstFamily -ne 'resident_host'" in script
    assert "$SummonAnywhereBlockerFamilyOrder -contains $CheckpointSummonEnablementGateFirstFamily" in script
    assert (
        "$CheckpointSummonEnablementGateHandoffFamilies -contains $CheckpointSummonEnablementGateFirstFamily"
    ) in script
    assert "$CheckpointSummonEnablementGateLegacyCoreHandoffObserved -or" in script
    assert "$CheckpointSummonEnablementGateCurrentCoreHandoffObserved" in script
    assert "$CheckpointSummonEnablementGateFirstFamilyHandoffObserved -and" in script
...
def test_lens_stage6_completion_audit_can_opt_into_launch_on_hotkey_runtime_readback() -> None:
    ...
    assert "$TrayPresenceApiExecutionProofScript" in script
    ...
    assert (
        "[string]$TrayPresenceApiExecutionProof.previous_next_smallest_truthful_gap "
        "-eq 'summon_tray_presence_blocker_boundary'" in script
    )
    assert (
        "[string]$TrayPresenceApiExecutionProof.next_smallest_truthful_gap "
        "-eq 'os_level_command_palette_binding'" in script
    )
    assert (
        "[string]$TrayPresenceApiExecutionProof.recommended_handoff_source "
        "-eq 'api_tray_presence_execution_global_hotkey_handoff'" in script
    )
    assert (
        "[string]$TrayPresenceApiExecutionProof.recommended_next_slice "
        "-eq 'prove_governed_os_binding_api_execution_after_tray_presence'" in script
    )
    assert (
        "[string]$TrayPresenceApiExecutionProof.recommended_proof_script "
        "-eq 'scripts/lens-os-binding-api-execution-proof.ps1 -Mode Status'" in script
    )
    assert "[bool]$TrayPresenceApiExecutionProof.tray_presence_started" in script
    assert "[bool]$TrayPresenceApiExecutionProof.tray_runtime_ready" in script
    assert "[bool]$TrayPresenceApiExecutionProof.tray_icon_visible" in script
    assert "[bool]$TrayPresenceApiExecutionProof.tray_presence_stop_observed" in script
    assert "-not [bool]$TrayPresenceApiExecutionProof.global_hotkey" in script
    assert "-not [bool]$TrayPresenceApiExecutionProof.overlay_window" in script
    assert "-not [bool]$TrayPresenceApiExecutionProof.summon_anywhere" in script
    assert "-not [bool]$TrayPresenceApiExecutionProofGovernance.hotkey_registration_authority" in script
    assert (
        "[string]$OsBindingApiExecutionProof.previous_next_smallest_truthful_gap "
        "-eq 'os_level_command_palette_binding'" in script
    )
    assert "[string]$OsBindingApiExecutionProof.route_next_smallest_truthful_gap -eq 'summon_binding'" in script
    assert (
        "[string]$OsBindingApiExecutionProof.next_smallest_truthful_gap "
        "-eq 'summon_overlay_window_blocker_boundary'" in script
    )
    assert (
        "[string]$OsBindingApiExecutionProof.recommended_handoff_source "
        "-eq 'api_os_binding_execution_overlay_window_handoff'" in script
    )
    assert (
        "[string]$OsBindingApiExecutionProof.recommended_next_slice "
        "-eq 'prove_governed_overlay_window_api_execution_after_global_hotkey_binding'" in script
    )
    assert (
        "[string]$OsBindingApiExecutionProof.recommended_proof_script "
        "-eq 'scripts/lens-overlay-api-execution-proof.ps1 -Mode Status'" in script
    )
    assert "[bool]$OsBindingApiExecutionProof.global_hotkey_bound" in script
    assert "[bool]$OsBindingApiExecutionProof.hotkey_runtime_ready" in script
    assert "[bool]$OsBindingApiExecutionProof.os_level_command_palette" in script
    assert "[bool]$OsBindingApiExecutionProof.hotkey_stop_observed" in script
    ...
    assert (
        "[string]$SummonApiBoundedExecutionProof.previous_next_smallest_truthful_gap "
        "-eq 'summon_binding_blocker_boundary'" in script
    )
    assert (
        "[string]$SummonApiBoundedExecutionProof.route_next_smallest_truthful_gap "
        "-eq 'summon_anywhere_runtime_readback'" in script
    )
    assert (
    ...
    assert (
        "[string]$SummonApiBoundedExecutionProof.recommended_handoff_source "
        "-eq 'api_summon_to_persistent_supervision_execution_handoff'" in script
    )
    assert (
        "[string]$SummonApiBoundedExecutionProof.recommended_next_slice "
        "-eq 'execute_persistent_supervision_api_enablement_after_bounded_summon_handoff'" in script
    )
    assert (
        "[string]$SummonApiBoundedExecutionProof.recommended_proof_script "
        "-eq 'scripts/lens-persistent-supervision-api-execution-proof.ps1 -Mode Status'" in script
    )
    assert "[bool]$SummonApiBoundedExecutionProof.summon_binding_observed" in script
    assert "[bool]$SummonApiBoundedExecutionProof.summon_runtime_ready" in script
    assert "[bool]$SummonApiBoundedExecutionProof.bounded_handoff_ready" in script
    assert "[bool]$SummonApiBoundedExecutionProof.local_open_ready" in script
    assert "-not [bool]$SummonApiBoundedExecutionProof.opened" in script
    assert "[bool]$SummonApiBoundedExecutionProof.no_launch" in script
    ...
    assert "$NextSmallestTruthfulGap -eq 'summon_anywhere_runtime_readback'" in script
    ...
    assert "'prove_governed_os_binding_api_execution_after_tray_presence'" in script
    ...
    assert "$ResidentSurfaceForegroundRuntimeProofRecommendedProofScript" in script
    assert "$ResidentSurfaceForegroundRuntimeProofRecommendedHandoff" in script
    assert "$ResidentSurfaceForegroundRuntimeProofResidentRuntimeAuthorityGrantHandoffObserved" in script
    assert "$ResidentSurfaceRuntimeSupervisionHandoffObserved = (" in script
    ...
    assert "recommended_handoff_source = $ResidentSurfaceForegroundRuntimeProofRecommendedHandoffSource" in script
    assert "recommended_next_slice = $ResidentSurfaceForegroundRuntimeProofRecommendedNextSlice" in script
    assert "recommended_handoff = $ResidentSurfaceForegroundRuntimeProofRecommendedHandoff" in script
    assert (
        "resident_surface_runtime_supervision_handoff_observed = "
        "$ResidentSurfaceRuntimeSupervisionHandoffObserved" in script
    )
    assert (
    ...
    assert "next_step = 'resolve_resident_claim_authority_before_persistent_supervision_resident_claim'" in script
    assert (
        "proof_script = 'scripts/lens-persistent-supervision-resident-claim-boundary-proof.ps1 -Mode Status'" in script
    )
    assert "authority_required = 'resident_claim_authority'" in script
    assert (
    ...
    assert "recommended_handoff_source = [string]$TrayPresenceApiExecutionProof.recommended_handoff_source" in script
    assert "recommended_handoff_source = [string]$OsBindingApiExecutionProof.recommended_handoff_source" in script
    assert "resident_runtime_api_execution_proof_readback = $ResidentRuntimeApiExecutionProofObserved" in script
    assert "tray_presence_api_execution_proof_readback = $TrayPresenceApiExecutionProofObserved" in script
    assert "os_binding_api_execution_proof_readback = $OsBindingApiExecutionProofObserved" in script
...
def test_lens_stage6_completion_audit_blocks_transition_without_authority() -> None:
    ...
    assert resident_runtime_boundary["overlay_control_authority_required"] == "overlay_control_authority"
    ...
    assert (
        resident_surface_foreground_runtime_proof["recommended_next_slice"]
    ...
    assert resident_surface_foreground_runtime_proof["authority_required"] == "process_supervision_authority"
    ...
    assert resident_surface_foreground_runtime_proof["resident_runtime_authority_grant_handoff_observed"] is True
    ...
    assert resident_surface_runtime_supervision_handoff["route"] == "/lens/resident-surface"
    assert (
        resident_surface_runtime_supervision_handoff["readiness_route"]
    ...
    assert payload["resident_surface_runtime_supervision_handoff_source"] == (
        "resident_surface_runtime_supervision_handoff"
    )
    assert payload["resident_surface_runtime_supervision_handoff"] == resident_surface_runtime_supervision_handoff
