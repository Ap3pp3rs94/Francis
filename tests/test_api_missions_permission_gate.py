...
def test_mission_create_denies_unscoped_actor_before_mutation(monkeypatch, tmp_path: Path) -> None:
    data_root = tmp_path / "francis_data"
    monkeypatch.setenv("FRANCIS_DATA_DIR", str(data_root))
    ...
    denied = client.post(
        "/missions/create",
        json={
            "objective": "Denied mission create",
            "summary": "Unscoped actor should not declare mission state.",
            "requester_id": "test.missions.permission_gate",
            "actor": _MISSION_ACTOR,
        },
    )
