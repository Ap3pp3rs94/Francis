...
def test_hud_root_serves_operator_surface() -> None:
    ...
    assert "Update Delivery" in response.text
    ...
    assert "Signing Posture" in response.text
...
def test_hud_root_supports_standalone_orb_window_mode() -> None:
    ...
    assert "const freeRoamGlide = clamp(" in response.text
    assert "roamPauseUntil: 0," in response.text
    assert "const arrivalRadius = Math.max(18, size * 0.08);" in response.text
    ...
    try:
        ...
        response = client.post(
            ...
            json={
                ...
                "cursor_x": 144,
                "cursor_y": 288,
                "target_stability_state": "settled",
                "target_stability_dwell_ms": 220,
                "target_stability_travel_px": 14,
                "target_stability_sample_count": 6,
                "window_x": 96,
                "window_y": 72,
                "window_width": 1280,
                "window_height": 760,
        ...
        assert body["target"]["stability"]["state"] == "settled"
