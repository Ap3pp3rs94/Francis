...
import services.orchestrator.app.routes.connectors as connector_routes
...
def test_connector_library_route_returns_structured_surface() -> None:
    with TestClient(app) as client:
        original_mode = _get_mode(client)
        original_scope = _get_scope(client)
        try:
            _set_mode(client, "observe", kill_switch=False)
            _set_scope(client, _enable_apps(original_scope, ["connectors"]))
            response = client.get("/connectors/library")
        finally:
    ...
    assert payload["library"]["connector_count"] >= 7


def test_connector_quarantine_and_revoke_workflow() -> None:
    workspace = Path(__file__).resolve().parents[2] / "workspace"
    repo_root = workspace.parent.resolve()
    registry_path = workspace / "connectors" / "registry.json"
    ...
    route_workspace_before = connector_routes._workspace_root
    route_repo_before = connector_routes._repo_root
    route_fs_before = connector_routes._fs
    route_ledger_before = connector_routes._ledger
    fs = WorkspaceFS(
    ...
    try:
        connector_routes._workspace_root = workspace.resolve()
        connector_routes._repo_root = repo_root
        connector_routes._fs = fs
        connector_routes._ledger = ledger
        ...
        with TestClient(app) as client:
            ...
            try:
                ...
                _set_scope(client, _enable_apps(original_scope, ["connectors", "approvals"]))

                quarantine = client.post(
                    "/connectors/discord/quarantine",
                    json={"reason": "Connector failed external review."},
                )
                assert quarantine.status_code == 200
                assert quarantine.json()["connector"]["status"] == "quarantined"

                request_revoke = client.post(
                    "/connectors/discord/revoke/request-approval",
                    json={"reason": "Connector should be revoked after quarantine."},
                )
                ...
                decision = client.post(
                    ...
                    json={"decision": "approved", "note": "Connector must be revoked."},
                )
                assert decision.status_code == 200

                revoke = client.post(
                    "/connectors/discord/revoke",
                    json={
        ...
        payload = revoke.json()
        assert payload["connector"]["status"] == "revoked"
        assert payload["approval_id"] == approval_id
        assert payload["presentation"]["kind"] == "connectors.lifecycle"
        registry = json.loads(_read_text(registry_path))
        discord = next(row for row in registry["connectors"] if row["id"] == "discord")
        assert discord["status"] == "revoked"
        assert discord["quarantined_at"]
        assert discord["revoked_at"]
        ledger_rows = _read_jsonl(run_ledger_path)
        revoke_receipt = next(
            row for row in reversed(ledger_rows) if str(row.get("kind", "")).strip() == "connectors.revoke"
        )
        quarantine_receipt = next(
            row for row in reversed(ledger_rows) if str(row.get("kind", "")).strip() == "connectors.quarantine"
        )
        ...
        assert revoke_summary["connector_id"] == "discord"
        assert revoke_summary["approval_id"] == approval_id
        assert quarantine_summary["connector_id"] == "discord"
        assert quarantine_summary["status"] == "quarantined"
    finally:
        connector_routes._workspace_root = route_workspace_before
        connector_routes._repo_root = route_repo_before
        connector_routes._fs = route_fs_before
        connector_routes._ledger = route_ledger_before
