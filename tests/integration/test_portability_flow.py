...
from tests.integration.workspace_state import PORTABILITY_RUNTIME_PATHS, isolated_workspace_files
...
def test_portability_export_preview_apply_flow() -> None:
    ...
    with isolated_workspace_files(PORTABILITY_RUNTIME_PATHS):
        try:
            ...
            paired_nodes = (
                topology_doc.get("paired_nodes", []) if isinstance(topology_doc.get("paired_nodes"), list) else []
            )
            assert paired_nodes
