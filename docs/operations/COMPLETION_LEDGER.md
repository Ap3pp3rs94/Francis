...
dispatch also cover roadmap-named `build_failure` and `validation_completion`
stimuli. Both trigger sources are now valid Reactor event sources and default
`classify` dispatch attempts record `reactor.dispatch.execution.receipt`,
verification receipt, and stable-return receipt with `route=classification`,
`stable_state=classification_recorded`, and source-specific outcomes
(`build_failure_classified` / `validation_completion_classified`). This is
classification/readback only. It does not start execution, decide approvals,
resolve deadletters, perform retries, send external messages, promote
capabilities, execute plugins, write memory, or add UI claims.

Latest targeted validation for the `2026-04-28` Reactor build/validation
classification dispatch slice:
...
### 2026-05-01 - Stage 6/Lens resident runtime process-supervision boundary readback

Stage 6/Lens now has a focused resident-runtime process-supervision boundary
diagnostic at
`scripts/lens-resident-runtime-process-supervision-boundary-proof.ps1`. The
proof consumes the resident runtime authority blocker split, verifies that the
first authority family is `process_supervision`, and proves that the boundary
remains readback-only: no local process launch, process supervision, process
restart, service control, tray registration, hotkey registration, overlay
control, memory write, approval-decision, or resident-claim authority is
granted.

The Stage 6 checkpoint now emits
`resident_runtime_process_supervision_boundary_proof` from the authority
blocker data it already collected. The completion audit consumes that readback
and advances its `next_smallest_truthful_gap` from
`resident_runtime_process_supervision_authority_boundary` to
`resident_runtime_service_control_authority_boundary`. This does not resolve or
grant process supervision; it proves the first resident-runtime authority family
as a bounded, non-mutating readback boundary and hands off to the next family
proof.

Stage 6 remains active and blocked. Stage 7/Telemetry is not started.

Latest targeted validation for the `2026-05-01` Stage 6/Lens resident runtime
process-supervision boundary readback:

- `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\lens-resident-runtime-process-supervision-boundary-proof.ps1 -Mode Status | ConvertFrom-Json | Select-Object kind,status,next_smallest_truthful_gap,process_supervision_boundary_observed,side_effects_denied,governance | ConvertTo-Json -Depth 8`
  Result: `passed; reported resident_runtime_service_control_authority_boundary with side_effects_denied`
- `python -m pytest tests\test_lens_resident_runtime_process_supervision_boundary_proof_script.py -q`
  Result: `passed`
- `python -m pytest tests\test_lens_stage6_checkpoint_script.py::test_lens_stage6_checkpoint_reports_blocked_done_criteria_without_authority tests\test_lens_stage6_completion_audit_script.py::test_lens_stage6_completion_audit_blocks_transition_without_authority -q`
  Result: `passed`
- `python -m pytest tests\test_lens_resident_runtime_process_supervision_boundary_proof_script.py tests\test_lens_resident_runtime_authority_blockers_proof_script.py tests\test_lens_stage6_checkpoint_script.py tests\test_lens_stage6_completion_audit_script.py -q`
  Result: `passed`
- `python -m ruff check tests\test_lens_resident_runtime_process_supervision_boundary_proof_script.py tests\test_lens_stage6_checkpoint_script.py tests\test_lens_stage6_completion_audit_script.py`
  Result: `passed`
- `python -m ruff format --check tests\test_lens_resident_runtime_process_supervision_boundary_proof_script.py tests\test_lens_stage6_checkpoint_script.py tests\test_lens_stage6_completion_audit_script.py`
  Result: `failed before formatting tests\test_lens_resident_runtime_process_supervision_boundary_proof_script.py; passed after formatting`

### 2026-05-01 - Stage 6/Lens resident runtime service-control boundary readback

Stage 6/Lens now has a focused resident-runtime service-control boundary
diagnostic at
`scripts/lens-resident-runtime-service-control-boundary-proof.ps1`. The proof
consumes the resident runtime authority blocker split, verifies that the second
authority family is `service_control`, and proves that the boundary remains
readback-only: no service install/control, local process launch, process
supervision, process restart, tray registration, hotkey registration, overlay
control, memory write, approval-decision, or resident-claim authority is
granted.

The Stage 6 checkpoint now emits
`resident_runtime_service_control_boundary_proof` from the authority blocker
data it already collected. The completion audit consumes that readback and
advances its `next_smallest_truthful_gap` from
`resident_runtime_service_control_authority_boundary` to
`resident_runtime_tray_presence_authority_boundary`. This does not resolve or
grant service installation/control; it proves the second resident-runtime
authority family as a bounded, non-mutating readback boundary and hands off to
the next family proof.

Stage 6 remains active and blocked. Stage 7/Telemetry is not started.

Latest targeted validation for the `2026-05-01` Stage 6/Lens resident runtime
service-control boundary readback:

- `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\lens-resident-runtime-service-control-boundary-proof.ps1 -Mode Status | ConvertFrom-Json | Select-Object kind,status,next_smallest_truthful_gap,service_control_boundary_observed,side_effects_denied,governance | ConvertTo-Json -Depth 8`
  Result: `passed; reported resident_runtime_tray_presence_authority_boundary with side_effects_denied`
- `python -m pytest tests\test_lens_resident_runtime_service_control_boundary_proof_script.py -q`
  Result: `passed`
- `python -m pytest tests\test_lens_stage6_checkpoint_script.py::test_lens_stage6_checkpoint_reports_blocked_done_criteria_without_authority tests\test_lens_stage6_completion_audit_script.py::test_lens_stage6_completion_audit_blocks_transition_without_authority -q`
  Result: `passed`
- `python -m pytest tests\test_lens_resident_runtime_service_control_boundary_proof_script.py tests\test_lens_resident_runtime_process_supervision_boundary_proof_script.py tests\test_lens_resident_runtime_authority_blockers_proof_script.py tests\test_lens_stage6_checkpoint_script.py tests\test_lens_stage6_completion_audit_script.py -q`
  Result: `passed`
- `python -m ruff check tests\test_lens_resident_runtime_service_control_boundary_proof_script.py tests\test_lens_resident_runtime_process_supervision_boundary_proof_script.py tests\test_lens_resident_runtime_authority_blockers_proof_script.py tests\test_lens_stage6_checkpoint_script.py tests\test_lens_stage6_completion_audit_script.py`
  Result: `passed`
- `python -m ruff format --check tests\test_lens_resident_runtime_service_control_boundary_proof_script.py tests\test_lens_resident_runtime_process_supervision_boundary_proof_script.py tests\test_lens_resident_runtime_authority_blockers_proof_script.py tests\test_lens_stage6_checkpoint_script.py tests\test_lens_stage6_completion_audit_script.py`
  Result: `failed before formatting tests\test_lens_stage6_completion_audit_script.py; passed after formatting`
- `git diff --check`
  Result: `passed; PowerShell LF-to-CRLF warnings only`
...
### 2026-05-02 - Stage 6/Lens persistent supervision execution authority proof

Stage 6/Lens now has a standalone persistent-supervision execution authority
proof diagnostic at
`scripts/lens-persistent-supervision-execution-authority-proof.ps1`, and the
Stage 6 completion audit consumes that proof directly. The proof uses a
temporary data root, creates the prerequisite host supervision authority grant,
creates the bounded persistent-supervision enablement authority grant, creates
and approves the exact persistent-supervision execution authority request, writes
the bounded execution authority grant receipt, reads the request/grant/readiness
and Lens status surfaces, and then calls the execution route to prove the grant
reaches the route but still stops at the resident-claim/runtime-readiness
boundary.

This is diagnostic/proof, audit/readback, and test work only. It exercises
existing governed API routes and writes only temporary diagnostic approvals and
authority receipts through existing route contracts. It does not grant product
execution authority, approval-decision authority, memory-write authority,
resident-claim authority, UI control, live process supervision, service control,
persistent runtime start, Stage 6 closure, or Stage 7 transition. The proof does
show that the route-level service-config write, persistent-supervision execution,
and receipt-write authority grant receipt is readable after the exact approved
request, while the attempted execution remains non-mutating and blocked by
`persistent_supervision_disabled`, `process_supervision_disabled`, and
`resident_claim_authority_not_granted`.

Stage 6 remains active and blocked. The completion audit now reports
`persistent_supervision_resident_claim_authority_boundary` as the next smallest
truthful gap after consuming both the enablement-authority proof and the
execution-authority proof. This narrows the handoff from "grant execution
authority or prove resident-claim boundary" to a direct resident-claim/runtime
readiness boundary without starting Stage 7.

Latest targeted validation for the `2026-05-02` Stage 6/Lens persistent
supervision execution authority proof and audit consumption:

- `pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\lens-persistent-supervision-execution-authority-proof.ps1 -Mode Status`
  Result: `passed`
- `python -m pytest tests\test_lens_persistent_supervision_execution_authority_proof_script.py -q`
  Result: `passed`
- `python -m pytest tests\test_lens_stage6_completion_audit_script.py::test_lens_stage6_completion_audit_blocks_transition_without_authority -q`
  Result: `passed`
- `python -m pytest tests\test_lens_persistent_supervision_execution_authority_proof_script.py tests\test_lens_stage6_completion_audit_script.py -q`
  Result: `passed`
- `python -m ruff check tests\test_lens_persistent_supervision_execution_authority_proof_script.py tests\test_lens_stage6_completion_audit_script.py`
  Result: `passed`
- `python -m ruff format --check tests\test_lens_persistent_supervision_execution_authority_proof_script.py tests\test_lens_stage6_completion_audit_script.py`
  Result: `failed before formatting the audit test; passed after formatting`
- `git diff --check`
  Result: `passed`
...
### 2026-05-02 - Stage 6/Lens resident host runtime loop execution denial boundary

Stage 6/Lens resident-host runtime readback now has a direct non-mutating
execution-denial boundary for the previous
`resident_host_runtime_loop_execution_denial_boundary` handoff.
`POST /lens/host/runtime-loop/execute` returns
`kind: lens.host.runtime_loop.execution_denial`, derives its blockers from the
existing `/lens/host/runtime-loop` contract, and proves that an attempted
runtime loop start remains denied when the resident runtime loop, supervision,
receipt-write, and resident-claim boundaries are not ready. `/lens/host` and
`/lens/status` embed the same denial readback so the operator surface can see
the boundary without treating it as execution.
...
does not add UI controls; and does not close Stage 6 or start Stage 7. Stage 6
remains active and blocked on the actual
`resident_host_runtime_loop_denial_receipt_readback` plus resident
host/runtime/summon surface and authority prerequisites.

Latest targeted validation for the `2026-05-02` Stage 6/Lens resident host
runtime loop execution denial boundary:

- `python -m pytest tests\test_api_lens.py::test_lens_host_runtime_loop_contract_stays_readback_only tests\test_api_lens.py::test_lens_host_runtime_loop_execution_denial_stays_non_mutating tests\test_api_lens.py::test_lens_api_observes_live_foreground_process_readback -q`
  Result: `passed`
- `python -m pytest tests\test_api_lens.py -q`
  Result: `passed`
- `python -m ruff check src\francis\lens\host_runtime_plan.py src\francis\lens\status.py src\francis\api\routes\lens.py src\francis\lens\__init__.py tests\test_api_lens.py`
...
- Stage 6/Lens has a backend readback contract and chat UI System/ORB readback
  ...
  memory-write, approval-decision, and resident-claim authority, plus a
  standalone persistent-supervision execution authority proof and completion-audit
  ...
  families, plus resident-runtime process-supervision boundary readback that
  keeps process launch, supervision, restart, service, tray, hotkey, overlay,
