...
$MannequinRecordInitializerPath = Join-Path $RepoRoot 'scripts\fr017-new-mannequin-interface-record.ps1'
...
$Output = [ordered]@{
  ...
  mannequin_input_template_path = $DefaultMannequinPath
  mannequin_record_initializer_path = $MannequinRecordInitializerPath
  mannequin_working_record_name_pattern = 'FR-017-MANNEQUIN-YYYY-MM-DD-PILOT-RECORD.json'
  ...
  mannequin_capture_runbook_contract = 'Use FR-017-MANNEQUIN-INTERFACE-INPUT-TEMPLATE.json with completed measurement and mockup records. Use scripts/fr017-new-mannequin-interface-record.ps1 only to create a real operator-supplied non-powered mannequin or arm-form interface working record after mockup readiness is ready. The template, initializer, and gate are operator input tooling only; they are not physical validation completion, pilot testing clearance, powered testing clearance, frame-coupled testing clearance, professional engineering approval, or FR-018 clearance.'
  mannequin_capture_plan_not_completion_evidence = $true
  next_required_mannequin_input = 'create_non_powered_mannequin_interface_record_with_fr017-new-mannequin-interface-record.ps1_then_rerun_mannequin_interface_gate'
