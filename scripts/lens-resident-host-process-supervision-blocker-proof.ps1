...
$Payload = [ordered]@{
  ...
  blocked_authority_required = [string](Get-PropertyValue -Payload $ProcessPayload -Name 'authority_required' -Default '')
  blocked_authority_granted = [bool](Get-PropertyValue -Payload $ProcessPayload -Name 'authority_granted' -Default $false)
  ...
  recommended_handoff = [ordered]@{
    ...
    previous_next_smallest_truthful_gap = 'resident_host_process_not_supervised'
    next_smallest_truthful_gap = 'stage6_lens_completion_audit'
    next_step = 'run_stage6_lens_completion_audit_after_process_supervision_handoff_readback'
    proof_script = 'scripts/lens-stage6-completion-audit.ps1 -Mode Status'
    route = '/lens/status'
    readiness_route = '/lens/status'
    acceptance_criterion = 'summon_anywhere'
    blocker = 'process_supervision_authority_not_granted'
    requirement_state = 'process_supervision_boundary_observed_without_authority'
    authority_required = 'none_new_stage6_completion_audit'
    authority_granted = $false
