...
$EngineeringReviewRecordInitializerPath = Join-Path $RepoRoot 'scripts\fr017-new-engineering-review-record.ps1'
...
$Output = [ordered]@{
  ...
  engineering_review_capture_runbook_contract = 'Use scripts/fr017-new-engineering-review-record.ps1 only to create a bounded operator-supplied professional engineering review working record linked to the same completed quick-release/cable-snag record. The initializer and template are input tooling only; they do not complete final physical validation and do not clear powered, frame-coupled, load-bearing, or FR-018 work.'
  ...
  next_required_engineering_review_input = 'create_professional_engineering_review_record_with_fr017-new-engineering-review-record.ps1_then_rerun_engineering_review_gate'
  engineering_review_input_template_path = $DefaultEngineeringReviewPath
  engineering_review_record_initializer_path = $EngineeringReviewRecordInitializerPath
  engineering_review_working_record_name_pattern = 'FR-017-ENGINEERING-REVIEW-YYYY-MM-DD-PILOT-RECORD.json'
