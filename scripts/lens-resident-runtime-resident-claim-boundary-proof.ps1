...
$ResidentSurfaceProof = if ($null -ne $CachedResidentSurfaceResult) {
  Get-PropertyValue -Payload $CachedResidentSurfaceResult -Name 'payload'
} else {
  $null
}
...
$ResidentSurfaceRuntimeProofObserved = (
  $null -ne $CachedResidentSurfaceResult -and
  [int](Get-PropertyValue -Payload $CachedResidentSurfaceResult -Name 'exit_code' -Default -1) -eq 0 -and
  [string](Get-PropertyValue -Payload $ResidentSurfaceProof -Name 'kind' -Default '') -eq 'lens.resident_surface.readiness_proof' -and
  [string](Get-PropertyValue -Payload $ResidentSurfaceProof -Name 'status' -Default '') -eq 'proof_passed' -and
  [bool](Get-PropertyValue -Payload $ResidentSurfaceProof -Name 'ok' -Default $false) -and
  [bool](Get-PropertyValue -Payload $ResidentSurfaceProof -Name 'resident_surface_resident_runtime_readback' -Default $false) -and
  [bool](Get-PropertyValue -Payload $ResidentSurfaceProof -Name 'resident_surface_resident_runtime_observed' -Default $false) -and
  [string](Get-PropertyValue -Payload $ResidentSurfaceProof -Name 'resident_surface_runtime_status' -Default '') -eq 'resident_runtime_observed' -and
  [bool](Get-PropertyValue -Payload $ResidentSurfaceProof -Name 'resident_surface_ready' -Default $false) -and
  [bool](Get-PropertyValue -Payload $ResidentSurfaceProof -Name 'resident_host_process' -Default $false) -and
  -not [bool](Get-PropertyValue -Payload $ResidentSurfaceProof -Name 'ready_for_lens_resident_claim' -Default $true) -and
  -not [bool](Get-PropertyValue -Payload $ResidentSurfaceProof -Name 'resident_claim_allowed' -Default $true) -and
  -not [bool](Get-PropertyValue -Payload $ResidentSurfaceProof -Name 'resident_claim_authority' -Default $true)
)
$ResidentSurfaceRuntimeMissingBoundaryObserved = $ResidentClaimBlockers -contains 'resident_surface_runtime_missing'
$EffectiveResidentClaimBlockers = if ($ResidentSurfaceRuntimeProofObserved) {
  @($ResidentClaimBlockers | Where-Object { $_ -ne 'resident_surface_runtime_missing' }) | Sort-Object -Unique
} else {
  @($ResidentClaimBlockers) | Sort-Object -Unique
}
$ResidentSurfaceRuntimeBoundaryObserved = $ResidentSurfaceRuntimeProofObserved -or $ResidentSurfaceRuntimeMissingBoundaryObserved
$ResidentClaimFamilyObserved = $AuthorityObserved -and $OverlayWindowObserved -and [string](Get-PropertyValue -Payload $ResidentClaimGroup -Name 'id' -Default '') -eq 'resident_claim' -and [string](Get-PropertyValue -Payload $ResidentClaimGroup -Name 'status' -Default '') -eq 'blocked' -and -not [bool](Get-PropertyValue -Payload $ResidentClaimGroup -Name 'ready' -Default $true) -and -not [bool](Get-PropertyValue -Payload $ResidentClaimGroup -Name 'authority_granted' -Default $true) -and -not [bool](Get-PropertyValue -Payload $ResidentClaimGroup -Name 'would_execute' -Default $true) -and [string](Get-PropertyValue -Payload $ResidentClaimGroup -Name 'route' -Default '') -eq '/lens/resident-runtime/plan' -and $EffectiveResidentClaimBlockers -contains 'resident_claim_authority_not_granted' -and $ResidentSurfaceRuntimeBoundaryObserved
...
$Checks = @(
  ...
  (New-Check -Id 'resident_surface_runtime_readback' -Status $(if ($ResidentSurfaceRuntimeProofObserved) { 'resident_runtime_observed' } elseif ($ResidentSurfaceRuntimeMissingBoundaryObserved) { 'missing_boundary' } else { 'missing_or_unexpected' }) -Passed $ResidentSurfaceRuntimeBoundaryObserved -Evidence 'scripts/lens-resident-surface-proof.ps1 -Mode Status' -Reason 'Resident-claim boundary must either keep the runtime-missing blocker or consume an explicit supervised resident-surface runtime proof.')
...
[ordered]@{
  ...
  resident_surface_runtime_proof_observed = $ResidentSurfaceRuntimeProofObserved; resident_surface_runtime_missing_boundary_observed = $ResidentSurfaceRuntimeMissingBoundaryObserved; resident_surface_resident_runtime_readback = [bool](Get-PropertyValue -Payload $ResidentSurfaceProof -Name 'resident_surface_resident_runtime_readback' -Default $false); resident_surface_resident_runtime_observed = [bool](Get-PropertyValue -Payload $ResidentSurfaceProof -Name 'resident_surface_resident_runtime_observed' -Default $false); resident_surface_runtime_status = [string](Get-PropertyValue -Payload $ResidentSurfaceProof -Name 'resident_surface_runtime_status' -Default ''); resident_surface_ready = [bool](Get-PropertyValue -Payload $ResidentSurfaceProof -Name 'resident_surface_ready' -Default $false); resident_surface_claim_allowed = [bool](Get-PropertyValue -Payload $ResidentSurfaceProof -Name 'resident_claim_allowed' -Default $false)
  cached_authority_blockers_proof = [bool](Get-PropertyValue -Payload $AuthorityResult -Name 'cached' -Default $false); cached_overlay_window_boundary_proof = [bool](Get-PropertyValue -Payload $OverlayWindowResult -Name 'cached' -Default $false); cached_resident_surface_proof = [bool](Get-PropertyValue -Payload $CachedResidentSurfaceResult -Name 'cached' -Default $false)
  ...
  resident_claim = [ordered]@{ status = [string](Get-PropertyValue -Payload $ResidentClaimGroup -Name 'status' -Default 'missing'); ready = [bool](Get-PropertyValue -Payload $ResidentClaimGroup -Name 'ready' -Default $false); authority_granted = [bool](Get-PropertyValue -Payload $ResidentClaimGroup -Name 'authority_granted' -Default $false); would_execute = [bool](Get-PropertyValue -Payload $ResidentClaimGroup -Name 'would_execute' -Default $false); route = [string](Get-PropertyValue -Payload $ResidentClaimGroup -Name 'route' -Default ''); evidence = [string[]]@(ConvertTo-StringArray -Value (Get-PropertyValue -Payload $ResidentClaimGroup -Name 'evidence' -Default @())); required_before = [string[]]@($ResidentClaimRequiredBefore); blockers = [string[]]@($EffectiveResidentClaimBlockers) }
  checks = @($Checks); blockers = [string[]]@($EffectiveResidentClaimBlockers); raw_resident_claim_blockers = [string[]]@($ResidentClaimBlockers); remaining_authority_families = [string[]]@($RemainingFamilies); remaining_authority_families_after_this_boundary = [string[]]@(); next_smallest_truthful_gap = 'stage6_lens_completion_audit'; evidence = @('scripts/lens-resident-runtime-authority-blockers-proof.ps1 -Mode Status', 'scripts/lens-resident-runtime-overlay-window-boundary-proof.ps1 -Mode Status', 'scripts/lens-resident-surface-proof.ps1 -Mode Status', '/lens/resident-runtime/plan', '/lens/resident-runtime/execute')
  governance = [ordered]@{ diagnostic_only = $true; wraps_existing_authority_blockers_proof = $true; overlay_window_boundary_readback = $OverlayWindowObserved; cached_resident_surface_proof_readback = ($null -ne $CachedResidentSurfaceResult); resident_surface_runtime_proof_readback = $ResidentSurfaceRuntimeProofObserved; product_execution_authority = $false; execution_authority = $false; approval_decision_authority = $false; local_process_launch_authority = $false; process_supervision_authority = $false; process_restart_authority = $false; service_install_authority = $false; service_control_authority = $false; tray_registration_authority = $false; tray_icon_authority = $false; notification_authority = $false; summon_authority = $false; hotkey_registration_authority = $false; overlay_control_authority = $false; window_management_authority = $false; capture_authority = $false; new_sensing_authority = $false; memory_write = $false; receipt_write_authority = $false; resident_claim_authority = $false; mutation_authority_granted = $false }
  message = if ($ResidentSurfaceRuntimeProofObserved) { 'The resident runtime resident-claim authority family consumed an explicit supervised resident-surface runtime proof; resident claim authority remains blocked and no process, service, tray, hotkey, overlay, memory, approval-decision, or resident-claim side effect is produced.' } else { 'The resident runtime resident-claim authority family is consumed as a readback boundary: resident claim remains blocked because supervised resident runtime is still missing, and no process, service, tray, hotkey, overlay, memory, approval-decision, or resident-claim side effect is produced.' }
