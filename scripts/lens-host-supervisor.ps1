...
$Payload = [ordered]@{
  ...
  recommended_handoff = [ordered]@{
    id = 'resident_host_process_supervision_authority_boundary'
    status = 'blocked'
    previous_next_smallest_truthful_gap = 'resident_host_process_supervision_authority_boundary'
    next_smallest_truthful_gap = 'host_supervision_authority_exact_approval_request'
    next_step = 'run_host_supervision_authority_request_proof_before_resident_start'
    proof_script = 'scripts/lens-host-supervision-authority-request-proof.ps1 -Mode Status'
    ...
    blockers = [string[]]@('process_supervision_authority_not_granted')
  }
  governance = [ordered]@{
    diagnostic_only = $true
