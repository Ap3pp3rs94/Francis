...
$ServiceControlRequiredBefore = ConvertTo-StringArray -Value (
  Get-PropertyValue -Payload $ServiceControlGroup -Name 'required_before' -Default @()
)
...
$RemainingFamiliesAfterThisBoundary = [string[]]@(
  $RemainingFamilies | Where-Object { $_ -ne 'process_supervision' -and $_ -ne 'service_control' }
)
...
$ProcessSupervisionFamilyObserved = (
  ...
  $ProcessSupervisionBlockers -contains 'process_supervision_authority_not_granted'
)
$ServiceControlFamilyObserved = (
  ...
  [string](Get-PropertyValue -Payload $ServiceControlGroup -Name 'route' -Default '') -eq '/lens/host/persistent-supervision/enablement' -and
...
$AuthorityBoundary = (
  ...
  -not [bool](Get-PropertyValue -Payload $AuthorityBlockersGovernance -Name 'service_install_authority' -Default $true) -and
...
$Checks = @(
  (New-Check -Id 'resident_runtime_authority_blockers_proof' -Status $(if ($AuthorityBlockersObserved) { 'proof_observed' } else { 'missing_or_failed' }) -Passed $AuthorityBlockersObserved -Evidence 'scripts/lens-resident-runtime-authority-blockers-proof.ps1 -Mode Status' -Reason 'The resident runtime authority blocker split proof must be observable before the service-control family can be consumed.')
  (New-Check -Id 'previous_process_supervision_family' -Status $(if ($ProcessSupervisionFamilyObserved) { 'blocked' } else { 'missing_or_unexpected' }) -Passed $ProcessSupervisionFamilyObserved -Evidence '/lens/host/supervision/authority/readiness' -Reason 'The previous process-supervision family must remain blocked before the service-control family is treated as the next boundary.')
  (New-Check -Id 'service_control_family' -Status $(if ($ServiceControlFamilyObserved) { 'blocked' } else { 'missing_or_unexpected' }) -Passed $ServiceControlFamilyObserved -Evidence '/lens/host/persistent-supervision/enablement' -Reason 'The second resident runtime authority family must be service install/control, and it must remain blocked.')
  (New-Check -Id 'service_control_side_effects_denied' -Status $(if ($BoundaryDenied) { 'denied_no_service_control' } else { 'unexpected_side_effect' }) -Passed $BoundaryDenied -Evidence 'resident_runtime_boundary_proof.would_*' -Reason 'The proof must not launch, supervise, control service/tray/hotkey/overlay, write memory, or claim resident status.')
  (New-Check -Id 'authority_boundary' -Status $(if ($AuthorityBoundary) { 'diagnostic_bounded' } else { 'unexpected_authority' }) -Passed $AuthorityBoundary -Evidence 'resident_runtime_authority_blockers.governance' -Reason 'This resident-runtime service-control proof must not grant product execution, process launch, process supervision, restart, service, tray, hotkey, overlay, memory, approval-decision, or resident-claim authority.')
)
...
$Payload = [ordered]@{
  ok = $ProofPassed
  kind = 'lens.resident_runtime.service_control_boundary.proof'
  ...
  service_control_boundary_observed = $ServiceControlFamilyObserved
  previous_process_supervision_family_observed = $ProcessSupervisionFamilyObserved
  authority_blockers_proof_observed = $AuthorityBlockersObserved
  side_effects_denied = $BoundaryDenied
  second_authority_family_consumed = $ServiceControlFamilyObserved
  ...
  service_control = [ordered]@{
    status = [string](Get-PropertyValue -Payload $ServiceControlGroup -Name 'status' -Default 'missing')
    ready = [bool](Get-PropertyValue -Payload $ServiceControlGroup -Name 'ready' -Default $false)
    authority_granted = [bool](Get-PropertyValue -Payload $ServiceControlGroup -Name 'authority_granted' -Default $false)
    would_execute = [bool](Get-PropertyValue -Payload $ServiceControlGroup -Name 'would_execute' -Default $false)
    route = [string](Get-PropertyValue -Payload $ServiceControlGroup -Name 'route' -Default '')
    evidence = [string[]]@(ConvertTo-StringArray -Value (Get-PropertyValue -Payload $ServiceControlGroup -Name 'evidence' -Default @()))
    required_before = [string[]]@($ServiceControlRequiredBefore)
    blockers = [string[]]@($ServiceControlBlockers)
  }
  checks = @($Checks)
  blockers = [string[]]@($ServiceControlBlockers)
  ...
  message = 'The resident runtime service-control authority family is consumed as a readback boundary: it remains blocked and produces no service install/control, process launch, supervision, restart, tray, hotkey, overlay, memory, approval-decision, or resident-claim authority.'
}
