...
function New-SummonAnywhereAuthorityRequestBundleOperatorHandoff {
  param(
    [AllowNull()]
    [object]$SourceHandoff,

    [AllowNull()]
    [object]$OsBindingAuthorityRequests,

    [AllowNull()]
    [object]$OverlayAuthorityRequests,

    [AllowNull()]
    [object]$SummonAuthorityRequests,
  ...
  $ApiBaseUrl = 'http://127.0.0.1:8000'
  $RuntimeReadbackCommand = '.\scripts\lens-summon-api-execution-proof.ps1 -Mode Status -AllowLaunchOnHotkey'
  $Definitions = @(
    [ordered]@{
      id = 'global_hotkey_binding'
      readback = $OsBindingAuthorityRequests
      ...
      live_effect = 'bounded global hotkey binding authority'
    }
    [ordered]@{
      id = 'overlay_window'
      readback = $OverlayAuthorityRequests
      ...
      live_effect = 'bounded overlay window authority'
    }
    [ordered]@{
      id = 'summon_binding'
      readback = $SummonAuthorityRequests
      ...
      live_effect = 'bounded local summon handoff authority'
    }
  )

  $ActiveGrantReceipts = [ordered]@{}
  $CurrentDefinition = $null
  foreach ($Definition in $Definitions) {
    $Readback = Get-PropertyValue -Payload $Definition -Name 'readback' -Default ([ordered]@{})
    $ActiveReceiptId = Get-AuthorityGrantActiveReceiptId -Readback $Readback
    if (-not [string]::IsNullOrWhiteSpace($ActiveReceiptId)) {
      $ActiveGrantReceipts[[string](Get-PropertyValue -Payload $Definition -Name 'id' -Default '')] = $ActiveReceiptId
      continue
    }
  ...
  if ($null -eq $CurrentDefinition) {
    return [ordered]@{
      ...
      next_operator_action = [ordered]@{
        ...
        live_effect = 'runs the explicit launch-on-hotkey summon runtime readback proof after exact authority grants are present'
        operator_supplied_values_required = $false
        requires_explicit_operator_opt_in = $true
        active_grant_receipts = $ActiveGrantReceipts
        script_would_execute = $true
        script_would_mutate = $true
        script_would_request_authority = $false
        script_would_grant_authority = $false
        script_would_decide_approval = $false
        script_would_launch_process = $true
        script_would_write_memory = $false
        script_would_claim_resident = $false
      }
      next_operator_command = [ordered]@{
        command = $RuntimeReadbackCommand
        mode = 'Status'
        requires_confirmation = $false
        requires_explicit_operator_opt_in = $true
        requires_actor = $false
  ...
  $CurrentReadback = Get-PropertyValue -Payload $CurrentDefinition -Name 'readback' -Default ([ordered]@{})
  $Requirement = [string](Get-PropertyValue -Payload $CurrentDefinition -Name 'requirement' -Default '')
  $RequestRoute = [string](Get-PropertyValue -Payload $CurrentReadback -Name 'request_route' -Default (
      Get-PropertyValue -Payload $CurrentDefinition -Name 'request_route' -Default ''
    ))
  $RequestsRoute = [string](Get-PropertyValue -Payload $CurrentReadback -Name 'route' -Default (
      Get-PropertyValue -Payload $CurrentDefinition -Name 'requests_route' -Default ''
    ))
  $GrantRoute = [string](Get-PropertyValue -Payload $CurrentReadback -Name 'authority_route' -Default (
      Get-PropertyValue -Payload $CurrentDefinition -Name 'grant_route' -Default ''
    ))
  $GrantsRoute = [string](Get-PropertyValue -Payload $CurrentReadback -Name 'grants_route' -Default (
      Get-PropertyValue -Payload $CurrentDefinition -Name 'grants_route' -Default ''
    ))
  $ExecuteRoute = [string](Get-PropertyValue -Payload $CurrentReadback -Name 'execute_route' -Default (
      Get-PropertyValue -Payload $CurrentDefinition -Name 'execute_route' -Default ''
    ))
  $ReadinessRoute = [string](Get-PropertyValue -Payload $CurrentDefinition -Name 'readiness_route' -Default '/lens/summon/readiness')
  $ApprovalAction = [string](Get-PropertyValue -Payload $CurrentReadback -Name 'action' -Default (
      Get-PropertyValue -Payload $CurrentDefinition -Name 'approval_action' -Default ''
    ))
  $AuthorityRequired = [string](Get-PropertyValue -Payload $CurrentDefinition -Name 'authority_required' -Default '')
  $ApprovedApprovalId = Get-ApprovalReadbackLatestApprovedId -Readback $CurrentReadback
  $PendingApprovalId = Get-ApprovalReadbackLatestPendingId -Readback $CurrentReadback
  ...
  $DecisionCommand = (
    "`$body = @{ id = '$PendingApprovalId'; action = '<approve|reject>'; comment = '<comment>'; actor = '<actor>' } | ConvertTo-Json -Compress; " +
    "Invoke-RestMethod -Method Post -Uri '$ApiBaseUrl/approvals/decision' -ContentType 'application/json' -Body `$body"
  )
  $GrantCommand = (
    "`$body = @{ approval_id = '$ApprovedApprovalId'; actor = '<actor>'; reason = '<reason>'; lease_seconds = 3600 } | ConvertTo-Json -Compress; " +
  ...
  if (-not [string]::IsNullOrWhiteSpace($PendingApprovalId)) {
    return [ordered]@{
      ...
      next_operator_action = [ordered]@{
        ...
        pending_approval_id = $PendingApprovalId
        mode = 'approval_wait'
        live_effect = "approval request exists for $LiveEffect; operator approval decision is required before grant receipt"
        operator_supplied_values_required = $false
        requires_operator_approval_decision = $true
        script_would_execute = $false
        script_would_mutate = $false
  ...
  return [ordered]@{
    ...
    next_operator_action = [ordered]@{
      ...
      requires = [string[]]@('system.write actor scope', 'explicit operator execution')
      mode = 'approval_request'
      live_effect = "creates a pending approval request for $LiveEffect only"
      operator_supplied_values_required = $true
      script_would_execute = $false
      script_would_mutate = $false
      script_would_request_authority = $true
...
$OsBindingAuthorityRequests = Get-PropertyValue -Payload $StatusReadback -Name 'os_binding_authority_requests' -Default ([ordered]@{})
$OverlayAuthorityRequests = Get-PropertyValue -Payload $StatusReadback -Name 'overlay_authority_requests' -Default ([ordered]@{})
$SummonAuthorityRequests = Get-PropertyValue -Payload $StatusReadback -Name 'summon_authority_requests' -Default ([ordered]@{})
...
$Stage6CompletionAuditPersistentSupervisionApiExecutionConcreteHandoffObserved = (
  $Stage6CompletionAuditReadbackObserved -and
  $Stage6CompletionAuditRecommendedConcreteHandoffSource -eq 'api_persistent_supervision_execution_handoff' -and
  @(
    'persistent_supervision_required_prerequisites_missing'
    'system_resident_presence_blockers'
  ) -contains [string](Get-PropertyValue -Payload $Stage6CompletionAudit -Name 'next_smallest_truthful_gap' -Default '') -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAudit -Name 'recommended_concrete_next_smallest_truthful_gap' -Default '') -eq 'stage6_lens_completion_audit' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'recommended_handoff_source' -Default '') -eq 'api_persistent_supervision_execution_handoff' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'status' -Default '') -eq 'audit_needed' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'previous_next_smallest_truthful_gap' -Default '') -eq 'persistent_supervision_execution_boundary' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'next_smallest_truthful_gap' -Default '') -eq 'stage6_lens_completion_audit' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'next_step' -Default '') -eq 'run_stage6_lens_completion_audit_after_persistent_supervision_api_execution' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'proof_script' -Default '') -eq 'scripts/lens-stage6-completion-audit.ps1 -Mode Status' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'route' -Default '') -eq '/lens/host/persistent-supervision/enablement/execution/apply' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'readiness_route' -Default '') -eq '/lens/host/persistent-supervision/enablement/execution/readiness' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'authority_required' -Default '') -eq 'none_new_stage6_completion_audit' -and
  ...
  -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'would_start_service' -Default $false) -and
  -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'would_write_memory' -Default $false) -and
  -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'would_claim_resident' -Default $false)
)
$Stage6CompletionAuditPersistentSupervisionTrayPrerequisiteRuntimeConcreteHandoffObserved = (
  $Stage6CompletionAuditReadbackObserved -and
  @(
    'api_tray_presence_execution_global_hotkey_handoff'
    'api_os_binding_execution_overlay_window_handoff'
    'api_overlay_execution_summon_binding_handoff'
    'api_summon_to_persistent_supervision_execution_handoff'
    'api_persistent_supervision_execution_handoff'
  ) -contains $Stage6CompletionAuditRecommendedConcreteHandoffSource -and
  @(
    'persistent_supervision_required_prerequisites_missing'
    'system_resident_presence_blockers'
  ) -contains [string](Get-PropertyValue -Payload $Stage6CompletionAudit -Name 'next_smallest_truthful_gap' -Default '') -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'consumed_persistent_supervision_prerequisite_first_missing' -Default '') -eq 'tray_presence' -and
  ...
  -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'authority_granted' -Default $true) -and
  [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'diagnostic_only' -Default $false)
)
...
$Stage6CompletionAuditRemainingAcceptanceHandoffObserved = (
  $Stage6CompletionAuditReadbackObserved -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAudit -Name 'recommended_handoff_source' -Default '') -eq 'stage6_remaining_acceptance_blockers_after_summon_runtime_readback' -and
  @('helpful_not_noisy_blockers', 'system_resident_presence_blockers') -contains [string](Get-PropertyValue -Payload $Stage6CompletionAudit -Name 'next_smallest_truthful_gap' -Default '') -and
  @(
  ...
  -not [bool](Get-PropertyValue -Payload $Stage6CompletionAudit -Name 'authority_granted' -Default $true) -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'status' -Default '') -eq 'blocked' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'next_smallest_truthful_gap' -Default '') -eq [string](Get-PropertyValue -Payload $Stage6CompletionAudit -Name 'next_smallest_truthful_gap' -Default '') -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'next_step' -Default '') -eq [string](Get-PropertyValue -Payload $Stage6CompletionAudit -Name 'recommended_next_slice' -Default '') -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'proof_script' -Default '') -eq 'scripts/lens-stage6-checkpoint.ps1 -Mode Status' -and
  @('helpful_not_noisy', 'system_resident_presence') -contains [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'acceptance_criterion' -Default '') -and
  [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'consumed_summon_anywhere_runtime_readback' -Default $false) -and
...
$Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff = Get-PropertyValue `
  -Payload $Stage6CompletionAudit `
  -Name 'acceptance_criterion_handoff' `
  -Default ([ordered]@{})
$Stage6CompletionAuditSystemResidentCurrentAcceptanceAction = Get-PropertyValue `
  -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff `
  -Name 'next_operator_action' `
  -Default ([ordered]@{})
$Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoffObserved = (
  $Stage6CompletionAuditReadbackObserved -and
  $Stage6CompletionAuditRemainingAcceptanceHandoffObserved -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAudit -Name 'next_smallest_truthful_gap' -Default '') -eq 'system_resident_presence_blockers' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff -Name 'status' -Default '') -eq 'persistent_supervision_enablement_applied' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff -Name 'previous_next_smallest_truthful_gap' -Default '') -eq 'system_resident_presence_blockers' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff -Name 'next_smallest_truthful_gap' -Default '') -eq 'persistent_supervision_execution_boundary' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff -Name 'next_step' -Default '') -eq 'run_stage6_prerequisite_bringup_review_persistent_supervision_enablement_receipt' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff -Name 'proof_script' -Default '') -eq 'scripts/lens-stage6-prerequisite-bringup-plan.ps1 -Mode Status' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff -Name 'acceptance_criterion' -Default '') -eq 'system_resident_presence' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff -Name 'next_operator_action_requirement' -Default '') -eq 'persistent_supervision_enablement_receipt' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceAction -Name 'id' -Default '') -eq 'review_persistent_supervision_enablement_receipt' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceAction -Name 'route' -Default '') -eq '/lens/host/persistent-supervision/enablement/executions' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceAction -Name 'mode' -Default '') -eq 'readback' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff -Name 'authority_required' -Default '') -eq 'none_readback_only' -and
  -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff -Name 'authority_granted' -Default $true) -and
  [bool](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff -Name 'read_only_contract' -Default $false) -and
  [bool](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff -Name 'diagnostic_only' -Default $false) -and
  -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff -Name 'would_execute' -Default $true) -and
  -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff -Name 'would_mutate' -Default $true) -and
  -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff -Name 'would_write_memory' -Default $true) -and
  -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff -Name 'would_decide_approval' -Default $true)
)
if ($Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoffObserved) {
  foreach ($Property in @($Stage6CompletionAuditSystemResidentCurrentAcceptanceHandoff.PSObject.Properties)) {
    $Stage6CompletionAuditSystemResidentAcceptanceConcreteHandoff[$Property.Name] = $Property.Value
  }
}
$Stage6CompletionAuditSystemResidentHostSupervisionAuthorityRequestProofConcreteObserved = (
  $Stage6CompletionAuditRecommendedConcreteHandoffSource -eq 'stage6_system_resident_host_supervision_authority_request_proof_handoff' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'previous_next_smallest_truthful_gap' -Default '') -eq 'host_supervision_authority_exact_approval_request' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'next_smallest_truthful_gap' -Default '') -eq 'persistent_supervision_required_prerequisites_missing' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'next_step' -Default '') -eq 'resolve_persistent_supervision_required_prerequisites_without_resident_claim' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'proof_script' -Default '') -eq 'scripts/lens-persistent-supervision-prerequisites-proof.ps1 -Mode Status' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'route' -Default '') -eq '/lens/host/persistent-supervision' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'readiness_route' -Default '') -eq '/lens/host/persistent-supervision/enablement' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'acceptance_criterion' -Default '') -eq 'system_resident_presence' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'authority_required' -Default '') -eq 'resident_host_process_tray_hotkey_overlay_summon_prerequisites' -and
  -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'authority_granted' -Default $true) -and
  [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'read_only_contract' -Default $false) -and
  [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'diagnostic_only' -Default $false) -and
  -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'would_execute' -Default $true) -and
...
$Stage6CompletionAuditSystemResidentAcceptanceSupersededByConcreteHandoff = (
  ...
  $Stage6CompletionAuditPersistentSupervisionTrayPrerequisiteRuntimeConcreteHandoffObserved -or
  (
    $Stage6CompletionAuditRecommendedConcreteHandoffSource -eq 'stage6_prerequisite_bringup_operator_plan_handoff' -and
    ...
    [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'next_smallest_truthful_gap' -Default '') -eq 'persistent_supervision_execution_boundary' -and
    [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'next_step' -Default '') -eq 'run_stage6_prerequisite_bringup_review_persistent_supervision_enablement_receipt' -and
    [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'proof_script' -Default '') -eq 'scripts/lens-stage6-prerequisite-bringup-plan.ps1 -Mode Status' -and
    [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'next_operator_action_requirement' -Default '') -eq 'persistent_supervision_enablement_receipt' -and
    [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'authority_required' -Default '') -eq 'none_readback_only' -and
    -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'authority_granted' -Default $true) -and
    [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'read_only_contract' -Default $false) -and
    [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'diagnostic_only' -Default $false) -and
    -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'would_execute' -Default $true) -and
    -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'would_mutate' -Default $true) -and
    -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'would_write_memory' -Default $true) -and
    -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedConcreteHandoff -Name 'would_decide_approval' -Default $true)
  )
)
$Stage6CompletionAuditReviewedSummonFirstBlockerFamily = [string](
  Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'first_blocker_family' -Default ''
)
$Stage6CompletionAuditReviewedSummonFirstBlockerResidentHostObserved = (
  $Stage6CompletionAuditReviewedSummonFirstBlockerFamily -eq 'resident_host' -and
  ...
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'authority_required' -Default '') -eq 'resident_runtime_execution_authority'
)
$Stage6CompletionAuditReviewedSummonFirstBlockerTrayPresenceObserved = (
  $Stage6CompletionAuditReviewedSummonFirstBlockerFamily -eq 'tray_presence' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'next_smallest_truthful_gap' -Default '') -eq 'summon_overlay_window_blocker_boundary' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'next_step' -Default '') -eq 'run_tray_presence_blocker_proof' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'proof_script' -Default '') -eq 'scripts/lens-summon-tray-presence-blocker-proof.ps1 -Mode Status' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'route' -Default '') -eq '/lens/tray' -and
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'readiness_route' -Default '') -eq '/lens/tray/readiness' -and
...
$Stage6CompletionAuditReviewedSummonFirstBlockerHandoffObserved = (
  ...
  [string](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'acceptance_criterion' -Default '') -eq 'summon_anywhere' -and
  -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'authority_granted' -Default $true) -and
  [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'read_only_contract' -Default $false) -and
  [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'diagnostic_only' -Default $false) -and
  -not [bool](Get-PropertyValue -Payload $Stage6CompletionAuditRecommendedHandoff -Name 'would_execute' -Default $true) -and
...
$Stage6CompletionAuditRecommendedHandoffConsumed = (
  ...
  $Stage6CompletionAuditPersistentSupervisionApiExecutionConcreteHandoffObserved -or
  $Stage6CompletionAuditPersistentSupervisionResidentClaimBoundaryHandoffObserved -or
  $Stage6CompletionAuditRemainingAcceptanceHandoffObserved -or
...
if (
  ...
  $Stage6CompletionAuditSystemResidentAcceptanceHandoffObserved -and
  -not $Stage6CompletionAuditSystemResidentAcceptanceSupersededByConcreteHandoff
) {
  $RecommendedConcreteHandoffSource = 'stage6_remaining_acceptance_system_resident_presence_handoff'
  $RecommendedConcreteHandoff = $Stage6CompletionAuditSystemResidentAcceptanceConcreteHandoff
} elseif (
  ...
  $RecommendedConcreteHandoffSource = 'persistent_supervision_first_missing_requirement_handoff'
  $RecommendedConcreteHandoff = $PersistentSupervisionFirstMissingRequirementHandoff
} elseif (
  $Stage6CompletionAuditRecommendedHandoffConsumed -and
  $Stage6CompletionAuditRemainingAcceptanceHandoffObserved -and
  $PersistentSupervisionResidentClaimBoundaryHandoffObserved -and
...
if ($Stage6CompletionAuditRuntimeReadbackRequired) {
  ...
  $RecommendedOperatorHandoff = New-SummonAnywhereAuthorityRequestBundleOperatorHandoff `
    -SourceHandoff $RecommendedHandoff `
    -OsBindingAuthorityRequests $OsBindingAuthorityRequests `
    -OverlayAuthorityRequests $OverlayAuthorityRequests `
    -SummonAuthorityRequests $SummonAuthorityRequests `
    -CompletionAuditJsonPath $ResolvedCompletionAuditJsonPath
...
$Stage6CompletionAuditSummonAuthorityRequestBundleHandoffObserved = (
  $Stage6CompletionAuditReviewedSummonAuthorityHandoffObserved -and
  [string](Get-PropertyValue -Payload $RecommendedOperatorHandoff -Name 'source' -Default '') -eq 'summon_anywhere_authority_request_bundle_handoff' -and
  @(
  ...
  -not [bool](Get-PropertyValue -Payload $RecommendedOperatorHandoff -Name 'approval_decision_authority' -Default $true) -and
  -not [bool](Get-PropertyValue -Payload $RecommendedOperatorHandoff -Name 'would_write_memory' -Default $false) -and
  -not [bool](Get-PropertyValue -Payload $RecommendedOperatorHandoff -Name 'would_claim_resident' -Default $false)
)
if (
  $Stage6CompletionAuditRecommendedHandoffConsumed -and
...
$Payload = [ordered]@{
  ...
  stage6_completion_audit_persistent_supervision_api_execution_concrete_handoff_observed = $Stage6CompletionAuditPersistentSupervisionApiExecutionConcreteHandoffObserved
  stage6_completion_audit_persistent_supervision_tray_prerequisite_runtime_concrete_handoff_observed = $Stage6CompletionAuditPersistentSupervisionTrayPrerequisiteRuntimeConcreteHandoffObserved
