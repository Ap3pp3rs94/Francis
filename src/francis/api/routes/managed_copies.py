...
from francis.managed_copies import (
    ...
    managed_copy_integrity_triage_disposition_snapshot,
    managed_copy_integrity_triage_dispositions_snapshot,
...
@router.post("/safe-delta-export-authorization-decision")
def safe_delta_export_authorization_decision(payload: dict[str, Any], request: Request) -> dict[str, Any]:
    actor = _managed_copy_write_actor(payload)
    decision = _write_permission(
        actor,
    ...
    if not decision.allowed:
        return _permission_denied(
            decision,
            required_scope=MANAGED_COPIES_SAFE_DELTA_EXPORT_AUTHORIZATION_DECISION_SCOPE,
            next_step="configure_actor_scope_before_deciding_safe_delta_export_authorization",
        )
    return managed_copy_safe_delta_export_authorization_decision_snapshot(payload, actor=actor)


@router.get("/safe-delta-export-authorization-decisions")
def safe_delta_export_authorization_decisions(
    copy_id: str = "", provisioning_receipt_id: str = "", isolation_verification_receipt_id: str = "", limit: int = 20
) -> dict[str, Any]:
    return managed_copy_safe_delta_export_authorization_decisions_snapshot(
        copy_id=copy_id,
        provisioning_receipt_id=provisioning_receipt_id,
        isolation_verification_receipt_id=isolation_verification_receipt_id,
        limit=limit,
...
@router.post("/safe-delta-export-artifact")
def safe_delta_export_artifact(payload: dict[str, Any], request: Request) -> dict[str, Any]:
    actor = _managed_copy_write_actor(payload)
    decision = _write_permission(
        actor,
    ...
    if not decision.allowed:
        return _permission_denied(
            decision,
            required_scope=MANAGED_COPIES_SAFE_DELTA_EXPORT_ARTIFACT_WRITE_SCOPE,
            next_step="configure_actor_scope_before_materializing_safe_delta_export_artifact",
        )
    return managed_copy_safe_delta_export_artifact_snapshot(payload, actor=actor)


@router.get("/safe-delta-export-artifact")
def safe_delta_export_artifact_readback(
    copy_id: str = "",
    provisioning_receipt_id: str = "",
    isolation_verification_receipt_id: str = "",
    ...
    return managed_copy_safe_delta_export_artifacts_snapshot(
        copy_id=copy_id,
        provisioning_receipt_id=provisioning_receipt_id,
        isolation_verification_receipt_id=isolation_verification_receipt_id,
...
@router.post("/integrity-triage-disposition")
def integrity_triage_disposition(payload: dict[str, Any], request: Request) -> dict[str, Any]:
    actor = _managed_copy_write_actor(payload)
    decision = _write_permission(
        actor,
        required_scope=MANAGED_COPIES_ROGUE_RECOVERY_WRITE_SCOPE,
    ...
    if not decision.allowed:
        return _permission_denied(
            decision,
            required_scope=MANAGED_COPIES_ROGUE_RECOVERY_WRITE_SCOPE,
            next_step="configure_actor_scope_before_recording_integrity_triage_disposition",
        )
    return managed_copy_integrity_triage_disposition_snapshot(payload, actor=actor)


@router.get("/integrity-triage-dispositions")
def integrity_triage_dispositions(
    copy_id: str = "", provisioning_receipt_id: str = "", isolation_verification_receipt_id: str = "", limit: int = 20
) -> dict[str, Any]:
    return managed_copy_integrity_triage_dispositions_snapshot(
        copy_id=copy_id,
        provisioning_receipt_id=provisioning_receipt_id,
        isolation_verification_receipt_id=isolation_verification_receipt_id,
        limit=limit,
