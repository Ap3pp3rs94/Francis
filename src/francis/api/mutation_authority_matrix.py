...
_RULES: tuple[AuthorityRule, ...] = (
    ...
    AuthorityRule(
        family="managed_copies_safe_delta_runtime_invocation_write",
        prefixes=("/managed-copies/safe-delta-runtime-invocation",),
        required_actor="payload.request_actor",
        required_scope="managed_copies.safe_delta.runtime_invocation.write",
        approval_requirement="first exact process-local pilot lease binding plus explicit invocation fingerprint confirmation",
        receipt_behavior="one immutable metadata-only invocation-result receipt",
        denial_behavior="api_permission_denied before payload processing, lineage inspection, directory creation, or invocation",
        governance_maturity="permission_gated_confirmed_local_invocation_only",
        notes="Consumes the first exact pilot-run binding and evaluates core-review handoff eligibility without network, connector, import, learning, core, or tenant-state mutation.",
    ),
    AuthorityRule(
        family="managed_copies_safe_delta_runtime_invocation_plan",
        prefixes=("/managed-copies/safe-delta-runtime-invocation-plan",),
        required_actor="payload.request_actor",
        required_scope="managed_copies.safe_delta.runtime_invocation.preflight",
        approval_requirement="exact current export-artifact receipt and artifact lineage",
        receipt_behavior="dry-run deterministic eligibility plan only",
        denial_behavior="api_permission_denied before artifact or lineage inspection",
        governance_maturity="permission_gated_dry_run_only",
        notes="Writes no receipt and performs no network, connector, import, learning, core, or tenant-state mutation.",
    ),
    AuthorityRule(
        family="managed_copies_safe_delta_runtime_source_write",
        prefixes=("/managed-copies/safe-delta-runtime-source",),
        required_actor="payload.request_actor",
        required_scope="managed_copies.safe_delta.runtime_source.write",
        approval_requirement="second exact process-local pilot lease binding plus explicit source fingerprint confirmation",
        receipt_behavior="one immutable canonical safe-delta source receipt; does not record final runtime evidence",
        denial_behavior="api_permission_denied before payload processing, lineage inspection, or receipt write",
        governance_maturity="permission_gated_confirmed_canonical_source_recording",
        notes="Consumes the second exact pilot-run binding after independently revalidating eligible invocation and complete safe-delta lineage.",
    ),
    AuthorityRule(
        family="managed_copies_safe_delta_runtime_source_plan",
        prefixes=("/managed-copies/safe-delta-runtime-source-plan",),
        required_actor="payload.request_actor",
        required_scope="managed_copies.safe_delta.runtime_source.preflight",
        approval_requirement="none; dry-run only",
        receipt_behavior="dry-run deterministic source plan only",
        denial_behavior="api_permission_denied before lineage inspection or filesystem work",
        governance_maturity="permission_gated_dry_run_only",
        notes="Consumes no lease binding and writes no source or runtime-evidence receipt.",
    ),
    AuthorityRule(
