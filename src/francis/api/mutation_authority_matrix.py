...
_RULES: tuple[AuthorityRule, ...] = (
    ...
    AuthorityRule(
        ...
        required_actor="payload actor where route accepts one",
        required_scope="documented_gap: no generic ApiPermissionGate scope enforced",
        approval_requirement="approval_store is used for approval-bound learn/quarantine actions",
        receipt_behavior="web_learning registry events and quarantine/request records",
        denial_behavior="route policy, validation, or approval mismatch response; not generic permission_gate",
        governance_maturity="documented_gap_policy_gated_without_api_scope",
        notes="Governance exists in route-specific policy/approval code, but not through ApiPermissionGate.",
    ),
    AuthorityRule(
        family="system",
        prefixes=("/system",),
    ...
    AuthorityRule(
        ...
        required_actor="payload.actor",
        required_scope="documented_gap: no memory.timeline.write ApiPermissionGate scope enforced",
        approval_requirement="not_required_currently",
        receipt_behavior="memory timeline event record",
        denial_behavior="validation or sanitized error; not generic permission_gate",
        governance_maturity="documented_gap",
        notes="Memory writes remain the next hardening target: provenance exists, but write authority is not yet gate-enforced.",
    ),
    AuthorityRule(
        family="missions",
        prefixes=("/missions",),
