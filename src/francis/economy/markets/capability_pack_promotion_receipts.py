...
__all__ = ["analyze_capability_pack_promotion_receipts"]

_STAGE17_CAPABILITY_ECONOMY_STAGE = "Stage 17 / Capability Economy"
_RECEIPT_ID_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,191}$")
_RECEIPT_PATH_MARKER = "/artifacts/plugins/promotions/"


def analyze_capability_pack_promotion_receipts(
    ...
    *,
    ...
    packs = _pack_promotion_receipts(packed)
    ...
    return {
        ...
        "available_promotion_receipt_count": len(available_ids),
        "available_promotion_receipt_path_count": len(available_paths),
        "packs": packs,
        "requirements": {
            "promotion_receipts_required_for_promoted": True,
            "promotion_receipt_ids_must_exist": True,
            "promotion_receipt_paths_must_stay_within_plugin_promotions": True,
            "promotion_receipt_bodies_not_read": True,
            "promotion_decisions_remain_separate_governed_actions": True,
        },
        "governance": {
            "read_only": True,
...
def _pack_promotion_receipts(entries: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for (pack_id, pack_version), grouped_entries in sorted(grouped.items()):
        blockers = _promotion_receipt_blockers(grouped_entries)
        failing = [entry for entry in sorted(grouped_entries, key=_entry_sort_key) if _entry_receipt_gaps(entry)]
        required_entries = [entry for entry in grouped_entries if entry["requires_promotion_receipt"]]
        present_ids = {
            entry["promotion_receipt_id"]
            for entry in required_entries
            if entry["promotion_receipt_present"] and entry["promotion_receipt_id"]
        }
        packs.append(
            {
                "pack_id": pack_id,
                ...
                "requires_promotion_receipt_count": len(required_entries),
                "promotion_receipt_present_count": sum(
                    1 for entry in required_entries if entry["promotion_receipt_present"]
                ),
                "promotion_receipt_missing_count": sum(
                    1 for entry in required_entries if "promotion_receipt_id_missing" in _entry_receipt_gaps(entry)
                ),
                "promotion_receipt_not_found_count": sum(
                    1 for entry in required_entries if "promotion_receipt_not_found" in _entry_receipt_gaps(entry)
                ),
                "promotion_receipt_invalid_count": sum(
                    1
                    for entry in required_entries
                    if any(gap in _entry_receipt_gaps(entry) for gap in _INVALID_RECEIPT_GAPS)
                ),
                "blockers": blockers,
                "promotion_receipt_ids": sorted(present_ids)[:25],
                "promotion_receipt_ids_truncated": len(present_ids) > 25,
...
_INVALID_RECEIPT_GAPS = ("promotion_receipt_id_invalid", "promotion_receipt_path_invalid")


def _promotion_receipt_blockers(entries: list[dict[str, Any]]) -> list[str]:
    blockers: list[str] = []
    if any(
        entry["requires_promotion_receipt"] and "promotion_receipt_id_missing" in _entry_receipt_gaps(entry)
        for entry in entries
    ):
        blockers.append("promotion_receipt_id_missing")
    if any("promotion_receipt_id_invalid" in _entry_receipt_gaps(entry) for entry in entries):
        blockers.append("promotion_receipt_id_invalid")
    if any("promotion_receipt_path_invalid" in _entry_receipt_gaps(entry) for entry in entries):
        blockers.append("promotion_receipt_path_invalid")
    if any(
        entry["requires_promotion_receipt"] and "promotion_receipt_not_found" in _entry_receipt_gaps(entry)
        for entry in entries
    ):
...
def _entry_receipt_gaps(entry: Mapping[str, Any]) -> list[str]:
    gaps: list[str] = []
    receipt_id = str(entry.get("promotion_receipt_id") or "")
    receipt_path = str(entry.get("promotion_receipt_path") or "")
    id_valid = bool(entry.get("promotion_receipt_id_valid"))
    path_valid = bool(entry.get("promotion_receipt_path_valid"))
    requires_receipt = bool(entry.get("requires_promotion_receipt"))
    ...
    if receipt_id and not id_valid:
        gaps.append("promotion_receipt_id_invalid")
    if receipt_path and not path_valid:
        gaps.append("promotion_receipt_path_invalid")
    if (
        requires_receipt
        and (receipt_id or receipt_path)
        and id_valid
        and path_valid
        and not entry.get("promotion_receipt_present")
    ):
...
def _entry_receipt_summary(entry: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "promotion_receipt_path": str(entry.get("promotion_receipt_path") or ""),
...
def _normalize_entry(
    ...
    *,
    ...
    receipt_id = _text(entry.get("promotion_receipt_id") or metadata.get("promotion_receipt_id"))
    receipt_path = _text(entry.get("promotion_receipt_path") or metadata.get("promotion_receipt_path"))
    receipt_id_valid = not receipt_id or _allowed_receipt_id(receipt_id)
    receipt_path_normalized = _normalize_receipt_path(receipt_path)
    receipt_path_valid = not receipt_path or _allowed_receipt_path(
    ...
    receipt_present = (
        ...
        and (
            (bool(receipt_id) and receipt_id in available_receipt_ids)
    ...
    return {
        ...
        "requires_promotion_receipt": status == "promoted",
        "promotion_receipt_id": receipt_id,
        "promotion_receipt_id_valid": receipt_id_valid,
        "promotion_receipt_path": receipt_path,
        "promotion_receipt_path_normalized": receipt_path_normalized,
        "promotion_receipt_path_valid": receipt_path_valid,
        "promotion_receipt_present": receipt_present,
...
def _allowed_receipt_path(path: str, *, available_receipt_paths: set[str]) -> bool:
    ...
    if not receipt_id or not _allowed_receipt_id(receipt_id):
        return False
    if path.startswith(("promotions/", "artifacts/plugins/promotions/", "data/artifacts/plugins/promotions/")):
        return True
