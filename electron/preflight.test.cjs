...
test("preflight reports nominal posture when writable roots and HUD runtime are healthy", () => {
  ...
  const state = buildPreflightState({
    ...
    provider: { severity: "low", summary: "Ollama is primary with governed remote fallback." },
    authority: { severity: "low", summary: "Authority posture is explicit and local-first." },
    signing: { severity: "low", summary: "Source checkout does not require Windows signing." },
...
test("preflight reports blocked posture when writable roots or runtime are unavailable", () => {
  const state = buildPreflightState({
    ...
    provider: { severity: "high", summary: "No model provider is configured." },
    authority: { severity: "high", summary: "Support authority is configured without a tenant or managed-copy binding." },
