...
* `Current Work Focus` - repo state, changed paths, blockers, latest terminal pressure, a structured terminal breakdown, the next actionable move, whether the current operator link is still active or stale, what the first terminal failure edge actually was, explicit evidence for why Lens picked the current move, a backend-owned approval-resume state so `Approve + Run` is described by the current-work contract instead of only by button text, and a backend-owned focus-action contract so the main next-move controls are no longer client-inferred
