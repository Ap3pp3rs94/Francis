...
* `Inbox Surface` and `Run Surface` - backend-shaped message and run surfaces now rendered in the HUD with current-vs-historical detail state, stronger message detail cards, and run continuity that carries the latest receipt summary directly into the run surface
