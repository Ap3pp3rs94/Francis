...
* `Capability Library` - a Stage 17 capability-economy surface with governed internal-library rows, explicit provenance classes (`Internal`, `Local Import`, `Vendor Provided`, `Third-Party`, plus quarantine/revocation posture), promotion approval state, backend-owned focus selection, compact audits, and explicit `Request Promotion Approval` / `Promote Capability` controls so staged packs become visible assets instead of hidden Forge residue
