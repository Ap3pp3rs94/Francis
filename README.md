...
* `release:publish:windows:public` and `overlay:dist:public` now fail fast through the signing doctor preflight, so a machine with only a self-issued dev signer does not burn a full Windows build before the public gate blocks it
