...
* the canonical Windows publish path is now the internal beta lane: `npm run release:publish:windows` runs the bounded hardening lane, auto-selects a safe machine-local signer when needed, and writes a labeled tester bundle under `dist/internal-beta/windows/current`
* the repo now distinguishes internal beta distribution from public-trust release packaging: `release:publish:windows` accepts a machine-local signer for QA / tester-only artifacts, while `release:publish:windows:public` additionally requires a configured publisher identity and rejects self-issued signers
...
* `release:publish:windows:public` and `overlay:dist:public` now fail fast through the signing doctor plus the repo-owned Windows sign-env verifier, so a machine without valid public-signing identity, tool paths, `metadata.json`, or `.NET 8` stops before the expensive packaging step
...
* public-trust verification now also requires `FRANCIS_WINDOWS_SIGNING_CHAIN_HINT`, so the expected non-leaf issuer/chain identity is pinned and a privately issued non-self-issued cert cannot silently satisfy the public gate
* the internal beta lane now emits `INTERNAL / QA / TESTER ONLY` metadata, relabeled artifacts, and a Windows tester handoff doc so machine-local signed bundles cannot reasonably be confused with future public-trust releases
...
* public Windows signing no longer depends on Electron Builder's built-in Azure Trusted Signing wrapper; the repo now owns the sign step directly through `build/sign/windows-sign.cjs`
* current estimated completion is `96%` overall, `96%` for feature/build coverage, and `95%` for production readiness
...
`pytest -q` remains the unbounded full-suite lane. `npm run test:full:first-failure` is the supported full-suite triage lane for the first concrete break, and `npm run test:full:stepwise` is the supported resume lane for continuing from that failure point without replaying the already-confirmed prefix. The commands above are the current hardening-smoke and distribution lanes for queue, inbox, telemetry, observer, observer-baseline normalization, security quarantine, inbox/presence state, autonomy housekeeping, overlay packaging posture, and Windows publish gating. `npm run release:hardening` runs the bounded release subset without changing the live workspace counters before vs. after the run, `npm run release:publish:windows` is now the canonical internal beta Windows publish lane and writes a labeled tester bundle under `dist/internal-beta/windows/current`, `npm run overlay:dist:public` now also runs both the public signing-doctor preflight and `npm run overlay:verify-windows-signing-env` before any Windows packaging work, `npm run release:publish:windows:public` adds publisher-name, expected chain-hint, and non-self-issued gating so a machine-local or privately issued signer cannot be mistaken for a public-trust release signer, and `npm run release:publish:windows:public:local` adds an ignored local env bootstrap path for the real publisher credentials. The public Windows sign step itself is now repo-owned through `build/sign/windows-sign.cjs`, not Electron Builder's Azure module wrapper. The autonomy reactor can now keep stale synthetic residue from reaccumulating through a budgeted housekeeping pass instead of depending only on manual repair, and the overlay packaging flow now emits an Authenticode verification manifest plus both signed and public-trust release gates.
...
Create isolated customer-specific copies without surrendering the core or pooling raw private data, with explicit creation, safe deltas, quarantine, and clean replacement.
